...
- **Fix 9f:** **Remove -fpack-struct from server executables:** Stop applying -fpack-struct to AuthServer, CharServer, etc. Packet headers (NtlPacketAU.h, etc.) use `#pragma pack(1)` in their own headers, so packet struct layout is correct without -fpack-struct on the TU. This fixes CNtlIniFile segfault. NtlShared still gets -fpack-struct for packet compilation.
