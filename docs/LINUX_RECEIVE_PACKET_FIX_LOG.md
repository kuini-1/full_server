...
- **Fix 9e:** **CNtlIniFile + -fpack-struct segfault:** MasterServer (built with -fpack-struct) uses CNtlIniFile with std::map m_iniData. Packed layout misaligns the map; destructor crashed in _Rb_tree::_S_right (corrupt tree pointers). **Fix:** (1) `#pragma pack(push, 8)` / `#pragma pack(pop)` around CNtlIniFile on Linux; (2) `alignas(8)` on m_iniData member to force proper alignment. **Rebuild:** Do a clean build (`ninja -C build -t clean && ninja -C build`) so header changes take effect.
