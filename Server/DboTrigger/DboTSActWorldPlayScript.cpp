...
void CDboTSActWorldPlayScript::TakeScriptDataForScript( CNtlTSScrProperty& clProperty )
{
	...
	NTL_SNPRINTF( g_NtlTSString, sizeof(g_NtlTSString), "%d", IsStart() ? 1 : 0 );
	clProperty.m_defProperty["start"] = g_NtlTSString;

	NTL_SNPRINTF( g_NtlTSString, sizeof(g_NtlTSString), "%d", IsUserOutFailed() ? 1 : 0 );
	clProperty.m_defProperty["uof"] = g_NtlTSString;

	NTL_SNPRINTF( g_NtlTSString, sizeof(g_NtlTSString), "%d", GetScriptID() );
