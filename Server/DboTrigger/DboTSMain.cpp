...
bool CDboTSMain::Create( void )
{
	...
	if ( !LoadFactories() )
	{
		printf( "Loading factories is failed. [%s]", TS_CODE_TRACE() );
		Delete();
		return false;
	}
	...
	if ( !LoadScripts() )
	{
		printf( "Loading the TS scripts is failed. [%s]", TS_CODE_TRACE() );
		Delete();
		return false;
	}
	...
	if ( !LoadEventMappers() )
	{
		printf( "Loading event mappers is failed. [%s]", TS_CODE_TRACE() );
		Delete();
		return false;
	}
	...
	if ( !LoadRecv() )
	{
		printf( "Loading receivers is failed. [%s]", TS_CODE_TRACE() );
		Delete();
		return false;
	}
