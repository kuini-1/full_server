...
void CopyCharState( sCHARSTATE* pDest, const sCHARSTATE* pSrc )
{
	...
	switch ( pSrc->sCharStateBase.aspectState.sAspectStateBase.byAspectStateId )
	{
...
DWORD Dbo_CalculatePowerLevel(
		WORD wLastStr, WORD wLastCon, WORD wLastFoc, WORD wLastDex, WORD wLastSol, WORD wLastEng,
...
{
	//float fA = (float)(wLastStr + wLastCon + wLastFoc + wLastDex + wLastSol + wLastEng);
