...
{
	TIMEQUEST_ROOM_STATE_REGISTRAITION,		// Enrollment
	TIMEQUEST_ROOM_STATE_WAITENTRY,			// Atmospheric entry
	TIMEQUEST_ROOM_STATE_PC_DIRECTION,		// PC enters production
	TIMEQUEST_ROOM_STATE_DAYRECORD_REQ,		// DayRecord to request a Query
	TIMEQUEST_ROOM_STATE_OBJECT_DIRECTION,	// OBJECT enters production
	TIMEQUEST_ROOM_STATE_PREPARE_WORLD,		// Books in the World WorldPool.
...
{
	HOBJECT					handle;
	WCHAR					awchName[ NTL_MAX_SIZE_CHAR_NAME + 1 ]; // 캐릭터 이름
...
{
	WCHAR					awchName[ NTL_MAX_SIZE_PARTY_NAME + 1 ];
};
