...
struct sCHARSEARCH_ENTITY
{
	...
	WCHAR			wszUserID[NTL_MAX_SIZE_USERID_UNICODE + 1];		// User ID
...
struct sPUNISHMENT_ENTITY
{
	...
	WCHAR					awchAccount[NTL_MAX_SIZE_USERID_UNICODE + 1]; // [7/16/2008 SGpro]
...
struct sGM_ENTITY
{
	...
	WCHAR					wszName[NTL_MAX_SIZE_TOOL_USERNAME_UNICODE + 1];		// User Name
	WCHAR					wszPartName[NTL_MAX_SIZE_TOOL_PARTNAME_UNICODE + 1];	// 소속
	WCHAR					wszComment[NTL_MAX_SIZE_TOOL_COMMENT_UNICODE + 1];		// 비고
...
struct sGM_GROUP_ENTITY
{
	...
	sGM_TOOL_AUTH_FLAG		sGMToolAuthFlag;								// GM Tool Authority
	sGM_COMMAND_AUTH_FLAG	sGMCommandAuthFlag;								// GM Command Authority
	sGM_ETC_AUTH_FLAG		sGMEtcAuthFlag;									// GM Etc Authority
};
