...
BEGIN_PROTOCOL(GU_CHAR_ACTION_ATTACK)
	...
	DWORD					dwLpEpEventId;
	BYTE					bChainAttack:1; // Whether chain attack
...
BEGIN_PROTOCOL(GU_ITEM_UPGRADE_RES)
	WORD			wResultCode;
	BYTE			byStack;			// After using number of stone stack -> stack changes
	BYTE			byGrade;			// Subject to change grades -> grade change
...
BEGIN_PROTOCOL(GU_SCOUTER_INDICATOR_RES)
	...
	WORD			wResultCode;
	DWORD			dwRetValue;
	WORD			wPhysicalOffence; //new
	WORD			wEnergyOffence; //new
	WORD			wPhysicalDefence; //new
	WORD			wEnergyDefence; //new
	BYTE			byRelationQuestCount;
	TBLIDX			aRelationQuest[30];
END_PROTOCOL()
//------------------------------------------------------------------
...
BEGIN_PROTOCOL(GU_SHOP_EVENTITEM_START_RES)
	HOBJECT			handle;
	BYTE			byType; //	0: Default 1: limited quantity sales event NPC
	WORD			wResultCode;
END_PROTOCOL()
//------------------------------------------------------------------
...
//------------------------------------------------------------------
BEGIN_PROTOCOL(GU_SCS_CHECK_START_REQ)
	BYTE			byPreFailCount;
	WORD			wCoverBitFlag;
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GU_SCS_REMAKE_RES)
	WORD			wCoverBitFlag;
	WORD			wResultCode;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GU_SCS_CHECK_OVERTIME_NFY)
END_PROTOCOL()
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GU_DROPITEM_INFO_RES)
	HOBJECT					hItemHandle;
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GU_CHAR_HISTORY_DATA_RES)
	BYTE					byCount;
	sTARGET_INFO_ANSWER		asCharHistoryData[NTL_MAX_CHAR_HISTORY_DATA_REQUEST];
END_PROTOCOL()
//------------------------------------------------------------------
...
HOBJECT			hTarget; //*(_DWORD *)(a2 + 2),
WORD			wResultCode; // *(_WORD *)(a2 + 6);
DWORD			dwUnknown; // *(_DWORD *)(a2 + 8),
WORD			awUnknown[2];// *(int **)(a2 + 12),
DWORD			dwRetValue; // *(_DWORD *)(a2 + 16),
WORD			awOffence[2]; // *(_DWORD *)(a2 + 20),
WORD			awDefence[2]; // *(_DWORD *)(a2 + 24),
DWORD			dwUnknown4; // *(_DWORD *)(a2 + 28),
DWORD			dwDodge;
DWORD			dwAttackRate;
END_PROTOCOL()
//------------------------------------------------------------------
...
WORD			wResultCode;
HOBJECT			ItemResultId[ITEM_DISASSEMBLE_MAX_RESULT];
END_PROTOCOL()
