...
#define NTL_MAX_SIZE_PETITIONCOUNSEL_USERID_UNICODE				( NTL_MAX_SIZE_USERID_UNICODE + NTL_MAX_SIZE_CHAR_NAME )
...
enum ePETITIONSYSTEM_CATEGORY_2
{
	...
	,PETITION_CATEGORY_2_BUG_SYSTEM // ?�???
...
struct sPETITIONDATA
{
	...
	ACCOUNTID								userAccountID_PetitionCreate; // Petition ??????? ?????? Account ID ( ?�??? : ?????? �?????? )
	CHARACTERID								userCharacterID_PetitionCreate;// Petition ??????? ?????? Char ID ( ?�??? : ?????? �?????? )
	GMACCOUNTID								gmAccountID_PetitionCreate; // Petition?? ?????? GM?? Account ID ( ?�??? : ???? ?????? - INVALID )
	CHARACTERID								gmCharacterID_PetitionCreate;// Petition?? ?????? GM?? Account ID ( ?�??? : ???? ?????? - INVALID )

	WCHAR									awchUserAccount_PetitionCreate[NTL_MAX_SIZE_USERID_UNICODE + 1];// Petition ??????? ?????? Account ID ( ?????? �?????? )
	WCHAR									awchUserCharacterName_PetitionCreate[NTL_MAX_SIZE_CHAR_NAME + 1];// Petition ??????? ?????? Char ID ( ?????? �?????? )
	...
	GMACCOUNTID								gmAccountID_Consult; // ???? ?????? GM Account ID( ?�??? )
