...
BEGIN_PROTOCOL(UG_CHAR_SKILL_REQ)
	...
	_compressedLocation			vFinalSubjectLoc; //new
...
BEGIN_PROTOCOL(UG_ITEM_UPGRADE_REQ)
	...
	BYTE				byStonPlace;		// Hoi Poi Stone Container Location
	BYTE				byStonPos;			// Hoi Poi stone location
	BYTE				byPosNum;			// Upgrade window combinations are: (slot number * 10 + Hoi Poi stone slot number of the item)
	BYTE				byCorePlace; //new
	BYTE				byCorePos; //new
END_PROTOCOL()
//------------------------------------------------------------------
...
END_PROTOCOL()
//------------------------------------------------------------------
//Still undecided (Do not use - removed possibility)
...
BEGIN_PROTOCOL( UG_MAIL_START_REQ )
	...
	eSCOUTER_PARTS_TYPE	byPartsType; //new
END_PROTOCOL()
//------------------------------------------------------------------
...
//------------------------------------------------------------------
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_SCS_CHECK_START_RES)
	bool		bRet;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_SCS_CHECK_RES)
	BYTE		byCount;
	bool		bRet;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_SCS_REPLY_REQ)
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_CASHITEM_UPDATE_DURATION_REQ)
	DWORD		dwProductId;
	HOBJECT		hItemHandle;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_CHAR_DEST_MOVE_SYNC)
	BYTE		byAvatarType;
	_compressedLocation		vCurLoc;
	_compressedDirection	vCurDir;
	_compressedLocation		vDestLoc;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_CHAR_NPCSERVER_MOVE_SYNC)
	BYTE		byAvatarType;
	_compressedLocation	vCurLoc;
END_PROTOCOL()
//------------------------------------------------------------------
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_MASCOT_SUMMON_REQ)
	TBLIDX			tblidx;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_MASCOT_UNSUMMON_REQ)
	TBLIDX			tblidx;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_MASCOT_DELETE_REQ)
	TBLIDX			tblidx;
END_PROTOCOL()
//------------------------------------------------------------------
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_CASHITEM_INFO_REQ)
	QWORD		biggestKnownProductId;
	BYTE		byMaxCount;
END_PROTOCOL()
//------------------------------------------------------------------
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_REQUEST_CHAR_HISTORY_DATA_REQ)
	BYTE		byCount;
	sTARGET_INFO_REQUEST	asRequestInfo[NTL_MAX_CHAR_HISTORY_DATA_REQUEST];
END_PROTOCOL()
//------------------------------------------------------------------
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_DROPITEM_INFO_REQ)
	HOBJECT		hItemHandle;
	DWORD		dwDialogID;
END_PROTOCOL()
//------------------------------------------------------------------
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_GAMEGUARD3_AUTH_ANSWER_RES)
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_PERFORMANCE_DATA_NFY)
	DWORD		dwValue;
	DWORD		adwCount[6];
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(UG_PERFORMANCE_DATA_2_NFY)
	DWORD		dwType;
	int			nValue;
END_PROTOCOL()
//------------------------------------------------------------------
