...
BEGIN_PROTOCOL( GQ_MAIL_START_REQ )
	...
	CHARACTERID					charID;
	eSCOUTER_PARTS_TYPE			byPartsType;
END_PROTOCOL()
//------------------------------------------------------------------
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GQ_GIFT_SHOP_BUY_REQ)
	HOBJECT					handle;
	CHARACTERID				charId;
	DWORD					dwGiftPoint;
	BYTE					byBuyCount;
...
BEGIN_PROTOCOL(GQ_TENKAICHIDAISIJYOU_PERIODEND_REQ)
	...
	BYTE			byMailType;
	char			chSystem[70 + 1];
	char			chText[NTL_MAX_LENGTH_OF_MAIL_MESSAGE_IN_MULTIBYTE + 1];
	int				nExpiredMinutes;
	unsigned int	nFee;
	HOBJECT			nHandle;
	BOOL			bList;
...
char			szReason[NTL_MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE + 1];
...
WCHAR			wchSenderName[NTL_MAX_SIZE_USERID_UNICODE + 1];
WCHAR			wchDestName[NTL_MAX_SIZE_USERID_UNICODE + 1];
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GQ_CAN_LOAD_CACHE) // by kuini can load cache
bool			canLoadCache;
bool			classChange;
CHARACTERID			charId;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GQ_CLASS_CHANGE) // by kuini class change
bool			classChange;
CHARACTERID			charId;
END_PROTOCOL()
