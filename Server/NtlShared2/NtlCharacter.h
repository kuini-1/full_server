...
enum eNPC_JOB
{
	...
	NPC_JOB_VENDING_MACHINE,				// sell items for zeni
	...
	NPC_JOB_ULTIMATE_DUNGEON_MANAGER,

	NPC_JOB_MASCOT_MERCHANT_2 = 55,
	NPC_JOB_MASCOT_GAMBLE_MERCHANT_2,	//mascot gamble npc.. get random mascot egg for zeni
	NPC_JOB_ITEM_EXCHANGE,
	NPC_JOB_SCOUTER_MERCHANT2,				//sell scouter for zeni

	NPC_JOB_SCOUTER_MERCHANT3 = 61,			//sell scouter for mudosa points
...
enum eNPC_FUNCTION_FLAG
{
	...
	NPC_FUNC_FLAG_EVENT_NPC					= MAKE_BIT_FLAG( NPC_FUNC_EVENT_NPC ),
	NPC_FUNC_FLAG_ITEM_EXCHANGE				= MAKE_BIT_FLAG( NPC_FUNC_ITEM_EXCHANGE ),
};
...
const int			DBO_CHAR_FACE_SHAPE_COUNT			= 15;

const int			DBO_CHAR_HAIR_SHAPE_COUNT			= 10;

const int			DBO_CHAR_HAIR_COLOR_COUNT			= 15;

const int			DBO_CHAR_SKIN_COLOR_COUNT			= 10;
...
const DWORD			DBO_CHAR_REPUTATION_DRAGONBALL = 100; // Dragon Ball myeongseongchi personal collection
...
{ 
	...
	sCHARSTATE_GUARD				sCharStateGuard; //new
...
struct sPC_PROFILE_LOCALIZE
{
	union
	{
		...
		sPC_PROFILE_LOCALIZE_CT pcProfileCT;
		sPC_PROFILE_LOCALIZE_SD_CH pcProfileSDCH;
	};
};
...
struct sPC_NEW_DATA // by kuini for loading fields from the database to game server
{
	DWORD			dwSpPointBought; // by kuini for SP purchase
};
struct sACC_NEW_DATA // by kuini for loading fields from the database to game server
{
	BYTE			bVipLevel;
};
...
struct sPC_BRIEF
{
	...
	float			fBaseFlySpeed;
	float			fLastFlySpeed;
	float			fBaseFlyDashSpeed;
	float			fLastFlyDashSpeed;
	float			fBaseFlyAccelSpeed;
...
struct sPC_PROFILE
{
	...
	TBLIDX			charTitle;
	sHOIPOIMIX_DATA sMixData;
	/////ABOVE THIS ALL IS RIGHT! BELOW THIS LINE NOT ALL RIGHT/////////
	bool			bIsGameMaster;		// true : 운영자 character

	GUILDID			guildId;	//correct

	BYTE			byUnknown; // dont know what/where this byte is supposed to be

	sPC_PROFILE_LOCALIZE sLocalize;
	BYTE			bySuperiorEffect;
	BYTE			bySuperiorType;
	BYTE			bySuperiorValue;
#ifndef _USE_HK_CLIENT_
	BYTE			byRankBattleRemainCount; //correct
#else
	BYTE			byRankBattleRemainCount; //correct
#endif // _USE_HK_CLIENT_
	bool			bIsInFreePvpZone;
	bool			bIsScrambleJoinFlag;
	bool			bIsScrambleRewardedFlag;
	BYTE			byHasBattleDragonBallBitFlag;
	...
	TBLIDX			mascotTblidx; 	//correct
	BYTE			byUnknown2; // dont know what this byte is supposed to be

	DWORD			dwWaguWaguPoints;	//correct
	bool			bInvisibleCostume;	//correct
};
...
DWORD Dbo_CalculatePowerLevel(
		WORD wLastStr, WORD byLastCon, WORD wLastFoc, WORD wLastDex, WORD wLastSol, WORD wLastEng,
