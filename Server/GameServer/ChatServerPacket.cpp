...
void	CChatServerSession::RecAuctionHouseSellRes(CNtlPacket * pPacket)
{
	...
	if (player && player->IsInitialized())
	{
		...
		res->dwMoney = req->dwFee;
		res->itemId = req->itemId;
		res->nEndSellTime = req->nEndSellTime;
		res->nItem = req->nItem;
		res->nStartSellTime = req->nStartSellTime;
...
void	CChatServerSession::RecAuctionHouseListData(CNtlPacket * pPacket)
{
	...
	if (player && player->IsInitialized())
	{
		...
		memcpy(&res->sData, &req->sData, sizeof(sTENKAICHIDAISIJYOU_DATA));
...
void CChatServerSession::RecvWaguCoinDecreaseNfy(CNtlPacket * pPacket)
{
	...
	if (player && player->IsInitialized())
	{
		player->UpdateWaguPoints(player->GetWaguPoints() + (player->GetWaguMachineCoin() - req->wWaguWaguCoin));
...
void	CChatServerSession::RecvDojoFunctionAddRes(CNtlPacket * pPacket)
{
	...
	if (player && player->IsInitialized())
	{
		...
		if (byNewDojoLevel > 0)
		{
			...
			packet0.SetPacketLen(sizeof(sGU_DOJO_LEVEL_CHANGED_NFY));
