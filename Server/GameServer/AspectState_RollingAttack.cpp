...
CAspectState_RollingAttack::CAspectState_RollingAttack(CCharacter *pParent)
	:CAspectState(ASPECTSTATE_ROLLING_ATTACK, pParent)
{
}

CAspectState_RollingAttack::~CAspectState_RollingAttack()
{
}

void CAspectState_RollingAttack::OnExit()
{
...
int CAspectState_RollingAttack::OnUpdate(DWORD dwTickDiff, float fMultiple)
{
	m_dwLastTransformEffect += dwTickDiff;

	float fDif = (m_pParent->GetLastRunSpeed() / m_pParent->GetWalkingSpeed() * 100.f) - 100.f;
...
//	printf("m_pParent->GetLastRunSpeed() %f, m_pParent->GetWalkingSpeed() %f, dwDelay: %u, fDif %f \n", m_pParent->GetLastRunSpeed(), m_pParent->GetWalkingSpeed(), dwDelay, fDif);
//	printf("m_pParent->GetCharStateID(): %u \n", m_pParent->GetCharStateID());
					...
					if (pWorldCellSibling)
					{
						...
						while (pTarget && pTarget->IsInitialized())
						{
							...
							if (m_pParent->IsInRange(pTarget, m_pParent->GetObjectRadius() + 3.0f))
							{
								...
								{
									m_pParent->SpecialAttackAction(pTarget, DBO_SPECIAL_ATTACK_TYPE_ROLLING_ATTACK, 1, m_pParent->GetSpecialSkillTblidx(), pSkill);
						...
						while (pMob && pMob->IsInitialized())
						{
							...
							if (m_pParent->IsInRange(pMob, m_pParent->GetObjectRadius() + 3.0f))
							{
								...
								{
									m_pParent->SpecialAttackAction(pMob, DBO_SPECIAL_ATTACK_TYPE_ROLLING_ATTACK, 1, m_pParent->GetSpecialSkillTblidx(), pSkill);
