...
void CExpEvent::TickProcess(DWORD dwTickDiff)
{
	...
	if (m_bOnOff == false)
	{
		if ((timeStruct.tm_wday == 5 && timeStruct.tm_hour >= 8) || timeStruct.tm_wday == 6) //check if its saturday or sunday -> start event
		{
			m_bOnOff = true;
	...
	{
		if (timeStruct.tm_wday == 0 && timeStruct.tm_hour >= 8) // check if its monday -> end event
		{
			EndEvent();
		}
	}
