...
void CMasterServerSession::RecvUserLogin(CNtlPacket * pPacket)
{
	...
	if(player && player->GetClientSession())
	{
		...
		{
			#if __LOG_CLIENT_SESSION__
				player->GetClientSession()->SetCharacterPCID(player->GetCharID());
				SYSTEMTIME ti;
				GetLocalTime( &ti );
				char m_CharFile[256];
				sprintf(m_CharFile,".\\logs\\gameserver\\session_log\\char_%d_channel_%d.log", player->GetCharID(), app->GetGsChannel());
				player->GetClientSession()->CreateLogFile(m_CharFile);
			#endif
