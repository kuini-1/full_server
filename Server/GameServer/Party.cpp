...
CParty::CParty()
{
	...
	m_byItemRank = DBO_PARTY_INVEN_DEFAULT_ITEM_RANK;
	m_registeredCharmTblidx[0] = INVALID_TBLIDX;
	m_registeredCharmTblidx[1] = INVALID_TBLIDX;
	m_adwRemainingTime[0] = INVALID_DWORD;
	m_adwRemainingTime[1] = INVALID_DWORD;
...
bool CParty::AddPartyMember(CPlayer* player)
{
	...
	memcpy(rPartyInfo->adwRemainingTime, m_adwRemainingTime, sizeof(rPartyInfo->adwRemainingTime));
	memcpy(rPartyInfo->registeredCharmTblidx, m_registeredCharmTblidx, sizeof(rPartyInfo->registeredCharmTblidx));
...
void CParty::SetMemberInfo(CPlayer* pPlayer, int nMemberCount)
{
	if (pPlayer)
	{
		...
		m_memberInfo[nMemberCount].bySuperiorEffect = pPlayer->GetSuperiorEffect();
...
void	CParty::SetMemberLP(CPlayer* player)
{
	for (int i = 0; i < m_byMemberInfoCount; i++)
	{
		if (m_memberInfo[i].hHandle == player->GetID())
		{
			...
			m_memberInfo[i].maxLp = player->GetLastMaxLP();
...
void CParty::SetMemberEP(CPlayer* player)
{
	for (int i = 0; i < m_byMemberInfoCount; i++)
	{
		if (m_memberInfo[i].hHandle == player->GetID())
		{
			...
			m_memberInfo[i].wMaxEP = player->GetLastMaxEP();
...
void	CParty::UpdateMemberLevel(CPlayer* player)
{
	...
	res->maxLp = player->GetLastMaxLP();
	res->wMaxEP = player->GetLastMaxEP();
...
void CParty::ReEnterBattleDungeon(CPlayer* pPartyLeader, CPlayer* pMember, CBattleDungeon* pDungeon, CWorld* pWorld, CScriptAlgoObject* pScript)//ccreconnect
{
	TBLIDX directPlayIdx = g_pTableContainer->GetServerConfigTable()->GetServerConfigData()->sBattleDungeonData.directPlay_EnterWorld;
	...
	if (pMember && pMember->IsInitialized())
	{
		...
		pMember->StartTeleport(pPartyLeader->GetCurLoc(), pPartyLeader->GetCurDir(), pWorld->GetID(), TELEPORT_TYPE_WORLD_MOVE, directPlayIdx, true);
...
{
	m_mapRewardSelectState.insert(std::make_pair(hPlayer, bySelectState));
...
{
	if (m_mapPartyInventory.size() >= NTL_PARTY_INVENTORY_SLOT_COUNT)
		return false;

	BYTE byInventoryPos = INVALID_BYTE;

	//Get free inventory pos
	for (BYTE i = 0; i < NTL_PARTY_INVENTORY_SLOT_COUNT; i++)
	{
	...
	memcpy(&partyItem.itemInfo.aOptionTblidx, pDrop->GetOptionSet(), sizeof(sITEM_OPTION_SET));
	...
	if (pDrop && pDrop->IsInitialized() && pDrop->GetCurWorld())
	{
		...
		if (m_diceProgress.hGainCharHandle != INVALID_HOBJECT)
		{
			...
			if (player && player->IsInitialized())
			{
				...
				else player->GetClientSession()->SendResultcode(818); // GAME_ITEM_INVEN_FULL
			}
		}
