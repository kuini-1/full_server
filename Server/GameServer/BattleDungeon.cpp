...
bool CBattleDungeon::Create(CPlayer* pPlayer, BYTE byStartStage)
{
	...
	*/
	...
	CWpsAlgoObject* pObj = (CWpsAlgoObject*)m_pWorld->GetScript(pWorld->wpsLinkIndex); //pWorld->wpsLinkIndex); by kuini testing CCBD
	if (pObj == NULL)
	{
...
bool CBattleDungeon::Rejoin(CPlayer* pPlayer) // ccreconnect
{
	/* Info:
		Battle Dungeon script hosted on World
	*/
	...
	//pObj->SetCCBDStartStage(GetCCBDCurStage());
...
void CBattleDungeon::LeaveDungeon(HOBJECT hPlayer)
{
	...
	{
	//	printf("CBattleDungeon::LeaveDungeon() no more players left \n");
