...
void CPlayer::event_LavaDamage()
{
	...
	int nDecreaseLP = (int)(GetLastMaxLP() * 0.10f);

	//printf("receive lava damage. Dmg %i Cur Life %i Max Life %i \n", nDecreaseLP, GetCurLP(), GetLastMaxLP());
...
// SCS CHECK EVENT
//--------------------------------------------------------------------------------------//
void CPlayer::event_SCSCheck()
{
	m_byPreFailCount += 1;

	printf("scs check fail count %u \n", m_byPreFailCount);

	if (m_byPreFailCount < 3)
	{
		//resent scs check start request
		StartSCSCheck();
	}
	else
	{
		//3x scs check failed !!! Disconnect player
		if (IsInitialized())
		{
			printf("SCS Check failed. DC Player..\n");
			this->GetClientSession()->Disconnect(false);
		}
	}
}

void CPlayer::Remove_event_SCSCheck()
{
	if (g_pEventMgr->HasEvent(this, EVENT_SCS_CHECK))
		g_pEventMgr->RemoveEvents(this, EVENT_SCS_CHECK);
}