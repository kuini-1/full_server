...
CControlScriptNodeAction* CAISNodeFactory::CreateControlNodeAction(const char* lpszNodeName)
{
	...
	std::unordered_map<std::string, CControlScriptNodeAction*(*)(const char*)>::iterator it = s_actionMap_0.find(lpszNodeName);
...
CControlScriptNodeCondition* CAISNodeFactory::CreateControlNodeCondition(const char* lpszNodeName)
{
	...
	std::unordered_map<std::string, CControlScriptNodeCondition*(*)(const char*)>::iterator it = s_conditionMap_0.find(lpszNodeName);
