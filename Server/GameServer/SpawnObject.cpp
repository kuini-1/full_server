...
bool CSpawnObject::IsInRange(CSpawnObject* pTarget, float fRange)
{
	...
	{
		float fOffSet = fRange + GetObjectRadius() + pTarget->GetObjectRadius();

		//if (IsPC())
		//	printf("fOffSet %f GetObjectRadius() %f GetObjectRadius() %f \n", fOffSet, GetObjectRadius(), pTarget->GetObjectRadius());

		return IsInRange(pTarget->GetCurLoc(), fOffSet);
	}
	return false;
}
...
bool CSpawnObject::IsInRange3(CSpawnObject* pTarget, float fRange)
{
	if (m_worldID == pTarget->GetWorldID())
	{
		...
		return IsInRange3(pTarget->GetCurLoc(), fOffSet);
	}
	return false;
}
