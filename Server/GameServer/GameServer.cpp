...
int CGameServer::OnInitApp()
{
	...
	CEventSystemEvent* pEventSystem = new CEventSystemEvent;
	pEventSystem->Create();
	UNREFERENCED_PARAMETER(pEventSystem);
	...
	//NTL_PRINT(PRINT_APP, "Winter Event System");
	//CWinterEvent* pWinterEvent = new CWinterEvent;
	//UNREFERENCED_PARAMETER(pScs);

	NTL_PRINT(PRINT_APP, "Battle Royale Event System");
	BattleRoyaleEvent* pBattleRoyaleEvent = new BattleRoyaleEvent;
	UNREFERENCED_PARAMETER(pBattleRoyaleEvent);
...
int	CGameServer::OnConfiguration(const char * lpszConfigFile)
{
	...
	if (!file.Read("GAMECONFIG", "ItemDropRate", m_config.OriginalItemDropRate))
	{
		return NTL_ERR_SYS_CONFIG_FILE_READ_FAIL;
	}
	...
	if (!file.Read("GAMECONFIG", "CraftExpRate", m_config.CraftExpRate))
	{
		return NTL_ERR_SYS_CONFIG_FILE_READ_FAIL;
	}
...
BOOL CGameServer::OnCommandInput(std::string& sCmd)
{
	...
	else if (sCmd == "5")
	{
		...
		g_pGameProcessor->StartServerShutdownNotice(5);
	}
	else if (sCmd == "10")
	{
		printf("Server shut down begin\n");
		g_pGameProcessor->StartServerShutdownNotice(10);
	}
	else if (sCmd == "15")
	{
		printf("Server shut down begin\n");
		g_pGameProcessor->StartServerShutdownNotice(15);
	}
	else if (sCmd == "20")
	{
		printf("Server shut down begin\n");
		g_pGameProcessor->StartServerShutdownNotice(20);
	}
	else if (sCmd == "25")
	{
		printf("Server shut down begin\n");
		g_pGameProcessor->StartServerShutdownNotice(25);
	}
	else if (sCmd == "30")
	{
		printf("Server shut down begin\n");
		g_pGameProcessor->StartServerShutdownNotice(30);
	}
	...
	else if (sCmd == "startwinterevent")
	{
		//g_pWinterEvent->StartEvent();
		//NTL_PRINT(PRINT_APP, "WinterEvent Started");
	}
	else if (sCmd == "stopwinterevent")
	{
		//g_pWinterEvent->EndEvent();
		//NTL_PRINT(PRINT_APP, "WinterEvent Stopped");
	}
	else if (sCmd == "1")
	{
		g_pBudokaiManager->StartSoloAdultBudokai();
...
void CGameServer::Init()
{
	...
	m_eventSoloExpBonus = 0;
	m_eventPartyExpBonus = 0;
	m_eventQuestExpBonus = 0;
	m_eventCraftExpBonus = 0;
	m_eventZeniDropBonus = 0;
	m_eventQuestMoneyBonus = 0;
	m_eventUpgradeRateBonus = 0;
	m_bMonsterAggressive = false;
	m_monsterStatBonusByWorldType.clear();
	ZeroMemory(&m_channelStatBonus, sizeof(m_channelStatBonus));
	ZeroMemory(&m_killDebuffConfig, sizeof(m_killDebuffConfig));
	m_killDebuffConfig.bEnable = false;
}
