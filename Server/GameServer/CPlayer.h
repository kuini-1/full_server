...
private:
	...
	DWORD					m_dwAccumulationNetP; //total netpy received
...
public:
	...
	void			RecvLoadPcDataRes(sPC_DATA* pPcData, sDBO_SERVER_CHANGE_INFO* pserverChangeInfo, bool bTutorialFlag, sCHAR_WAR_FOG_FLAG* pWarFogInfo, sMAIL_NEW_BREIF* pMailBrief, sRANKBATTLE_SCORE_INFO* pRankBattleScore, BYTE* pbyTitleIndexFlag, WORD wWaguCoin, WORD wEventCoin, sPC_NEW_DATA* pPcNewData, sACC_NEW_DATA* pAccNewData);
	...
	void			SendCurrentSkillInfo();
	void			SendCurrentHtbSkillInfo();
	...
	inline DWORD		GetSkillPointsBought() { return SpPointsBought; } 
	inline void			SetSkillPointsBought(DWORD sp) { SpPointsBought = sp; } 

	inline BYTE			GetVipLevel() { return bVipLevel; }
	inline void			SetVipLevel(BYTE val) { bVipLevel = val; }
	...
	virtual TBLIDX*			GetClassSkills(BYTE byClass, BYTE byNewClass, BYTE returnType, TBLIDX* arr);
	...
	inline bool				IsTutorial()  { return bIsTutorial; }
	inline bool				SetTutorial(bool b) { return bIsTutorial = b; }
	...
	inline void				SetSuperiorEffect(BYTE effect) { player_data.bySuperiorEffect = effect; }
	inline BYTE				GetSuperiorEffect() const	{ return player_data.bySuperiorEffect; }
	...
	inline DWORD		GetNetpyTokens() 		{ return player_data.sLocalize.pcProfileCT.timeP; }
	inline void			SetNetpyTokens(DWORD points)	{ player_data.sLocalize.pcProfileCT.timeP = points; }
	void				UpdateNetpyToken(bool bIsTimer, DWORD dwAddNetpy, bool bIsAdd);
	...
	void				UpdateClassItem(BYTE byClass, BYTE currentClass);
	inline bool			IsNamek(BYTE byClass);
	inline bool			IsMajin(BYTE byClass);
	inline bool			IsHuman(BYTE byClass);
	...
	//RECEIVE NETPY POINTS
	void				NetpyEvent(DWORD dwTickDiff);
...
// MONEY
public:
	void			UpdateZeni(BYTE ChangeType, DWORD dwAmount, bool bIsNew, bool bDoQuery = true);
...
// FREE BATTLE
private:
	CHARACTERID		uiFreeBattleTarget;
	bool			bIsFreeBattle;
	DWORD			dwFreeBattleID;
	bool			bIsOutsideBattle;
	/************************************************************************/

	// by kuini

	DWORD			SpPointsBought;
	BYTE			bVipLevel;
...
public:
	...
	inline void			SetFreeBattleTarget(CHARACTERID ID)	{ uiFreeBattleTarget = ID; }
	inline CHARACTERID	GetFreeBattleTarget()	const	{ return uiFreeBattleTarget; }
	inline void			SetPcIsFreeBattle(bool status)	{ bIsFreeBattle = status; }
	inline bool			GetPcIsFreeBattle()				{ return bIsFreeBattle; }
	inline void			SetFreeBattleID(DWORD ID)		{ dwFreeBattleID = ID; }
	inline DWORD		GetFreeBattleID()				{ return dwFreeBattleID; }

	inline void			SetOutsideFreeBattle(bool status){ bIsOutsideBattle = status; }
	inline bool			IsOutsideFreeBattle()			{ return bIsOutsideBattle; }

	inline void			InitFreeBattle()	{ uiFreeBattleTarget = INVALID_CHARACTERID; bIsFreeBattle = false; dwFreeBattleID = INVALID_DWORD; bIsOutsideBattle = false; }
...
// HOI POIT CRAFTING
public:
	inline void			SetHoiPoiMixJob(BYTE job)		{ player_data.sMixData.bySpecialType = job; }
	inline void			IncreaseHoiPoiMixLv(BYTE level)	{ player_data.sMixData.byMixLevel = level; }
	inline BYTE			GetHoiPoiMixLv()				{ return player_data.sMixData.byMixLevel; }
	inline void			SetHoiPoiMixExp(DWORD exp)		{ player_data.sMixData.dwMixExp = exp; }
	inline DWORD		GetHoiPoiMixExp()				{ return player_data.sMixData.dwMixExp; }
	inline void			ResetHoiPoiMixData()			{
		player_data.sMixData.bySpecialType = 0;
		player_data.sMixData.bSpecialStart = false;
	}

	inline BYTE			GetHoiPoiMixJob()		{ return player_data.sMixData.bySpecialType; }
	inline bool			GetHoiPoiNormal()		{ return player_data.sMixData.bNormalStart; }
	inline bool			GetHoiPoiSpecial()		{ return player_data.sMixData.bSpecialStart; }
	inline void			SetHoiPoiNormal(bool status)	{ player_data.sMixData.bNormalStart = status; }
	inline void			SetHoiPoiSpecial(bool status)	{ player_data.sMixData.bSpecialStart = status; }
...
private:
	...
	DWORD									dwLastMailReloadStatistic;	//allow manual reloading once every minute

	DWORD									dwLastMailReload;	//allow manual reloading once every minute

	DWORD									dwLastMailLoad;		//only do database query once every 1 minute to avoid lots of queries when player keep opening mail
...
public:
	...
	void					LoadMails(HOBJECT hObject, eSCOUTER_PARTS_TYPE ePartsType);
...
public:
	...
	void					SetIsQuickTeleportLoaded(bool b)	{ bIsQuickTeleportLoaded = b; }
	bool					IsQuickTeleportLoaded()				{ return bIsQuickTeleportLoaded; }
...
public:
	...
	inline void		SetVehicleEngine(bool bFlag)	{ m_vehicle.bIsVehicleEngineOn = bFlag; }
	inline bool		GetVehicleEngine() { return m_vehicle.bIsVehicleEngineOn; }
...
public:
	...
	void			SetCCBDRejoin(bool id) { m_bCCBDRejoin = id; }
	bool			GetCCBDRejoin() { return m_bCCBDRejoin; }
...
private:
	...
	GUILDID			m_guildId;
	WCHAR			m_wszGuildName[NTL_MAX_SIZE_GUILD_NAME_IN_UNICODE + 1];
...
public:
	...
	void				SetAllCharTitle(unsigned char* flags){ memcpy(TitleIndexFlag, flags, sizeof(TitleIndexFlag)); }
	unsigned char*		GetAllCharTitle() { return TitleIndexFlag; }
...
private:
	bool				m_bIsSCSCheck;
	BYTE				m_byPreFailCount;

public:

	inline void			SetSCSPreFailCount(BYTE byCount) { m_byPreFailCount = byCount; }
	inline BYTE			GetSCSPreFailCount() { return m_byPreFailCount; }

	inline void			SetSCSCheckBegin(bool b) { m_bIsSCSCheck = b; }
	inline BYTE			GetSCSCheckBegin() { return m_bIsSCSCheck; }
