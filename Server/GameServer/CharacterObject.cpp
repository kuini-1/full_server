...
{
	if (GetTickCount() < m_dwNextAttackTime) //check if can attack
	{
		return false;
	}
	else if (!IsStanding())
...
bool CCharacterObject::UpdateCurLP(int lpDiff, bool bIncrease, bool bAutoRecover/*, sLP_TRACE_DATA *rLpTraceData*/)
{
	if (bIncrease)
	{
		return SetCurLP(Dbo_IncreaseDboLp(GetCurLP(), lpDiff, GetLastMaxLP()));
	}
	else
	{
		return SetCurLP(Dbo_DecreaseDboLp(GetCurLP(), lpDiff, GetLastMaxLP()));
	}
...
bool CCharacterObject::UpdateCurEP(WORD wEpDiff, bool bIncrease, bool bAutoRecover)
{
	if (bIncrease)
	{
		return SetCurEP(Dbo_IncreaseDboEp(GetCurEP(), wEpDiff, GetLastMaxEP()));
	}
	else
	{
		return SetCurEP(Dbo_DecreaseDboEp(GetCurEP(), wEpDiff, GetLastMaxEP()));
	}
...
bool CCharacterObject::UpdateCurRP(WORD wRpDiff, bool bIncrease, bool bHitCharge)
{
	if (bIncrease)
	{
		...
		{
			//printf("wRpDiff %u GetCurRP() %u GetCharAtt()->GetLastMaxRP() %u\n", wRpDiff, GetCurRP(), GetCharAtt()->GetLastMaxRP());
			if (wRpDiff + GetCurRP() < GetCharAtt()->GetLastMaxRP())
			{
				return SetCurRP(UnsignedSafeIncrease<WORD>(GetCurRP(), wRpDiff));
			}
			else
			{
				int nAddRpBall = wRpDiff / GetCharAtt()->GetLastMaxRP();
				if (nAddRpBall <= 0)
					nAddRpBall = 1;
				...
				if (GetCurRPBall() == GetMaxRPBall())
					...
					SetCurRP((GetCurRP() + wRpDiff) - (nAddRpBall * GetCharAtt()->GetLastMaxRP()));
...
{
	if (bIncrease)
		return SetCurAP(Dbo_IncreaseDboAp(GetCurAP(), apDiff, GetCharAtt()->GetLastMaxAP()));
	else
		return SetCurAP(Dbo_DecreaseDboAp(GetCurAP(), apDiff, GetCharAtt()->GetLastMaxAP()));
...
{
	return (float)GetCurLP() <= (float)GetLastMaxLP() * fLowLPFactor / 100.0f;
}


bool CCharacterObject::ConsiderEPLow(float fLowLPFactor)
{
	return (float)GetCurEP() <= (float)GetLastMaxEP() * fLowLPFactor / 100.0f;
}
...
void CCharacterObject::UpdateMoveSpeed(float fWalkSpeed, float fRunSpeed)
{
	...
	res->fLastFlyingSpeed = GetLastFlySpeed();
	res->fLastFlyingDashSpeed = GetLastFlyDashSpeed();
	res->fLastFlyingAccelSpeed = GetLastFlyAccelSpeed();
	Broadcast(&packet);

	SetLastRunSpeed(res->fLastRunningSpeed);
}
//--------------------------------------------------------------------------------------//
...
void CCharacterObject::UpdateAttackSpeed(WORD wAttackSpeed)
{
	...
	SetLastAttackSpeedRate(res->wAttackSpeedRate);
}

void CCharacterObject::BlockMovementSpeed()
{
	m_fWalkSpeedBeforeBlock = GetWalkingSpeed();
	m_fRunSpeedBeforeBlock = GetLastRunSpeed();
...
void CCharacterObject::UndoMovementBlock()
{
	...
	SetLastRunSpeed(m_fRunSpeedBeforeBlock);
}
...
{
	return GetObjectRadius() + GetCharAtt()->GetLastAttackRange();
}
