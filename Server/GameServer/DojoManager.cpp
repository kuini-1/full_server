...
void CDojo::TickProcess(DWORD dwTickDiff)
{
	...
	switch (m_matchStateInfo.eState)
	{
		case eDBO_DOJO_STATUS_NORMAL:
		{
			...
			m_dwNextUpdate = 1000 * 900; // 15 minutes
			m_matchStateInfo.tmNextStepTime = curTime + 900; // 900 seconds = 15 minutes
		...
		case eDBO_DOJO_STATUS_RECEIVE:
		{
			...
			//this is only called if no dojo war has been accepted or requested (15min time run out)
