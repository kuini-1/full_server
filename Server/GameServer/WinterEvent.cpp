#include "stdafx.h"
#include "WinterEvent.h"
#include "GameServer.h"
#include "NtlPacketGU.h"
#include "NtlStringW.h"
#include "ObjectManager.h"
#include "NtlAdmin.h"
#include "Monster.h"
#include "NtlRandom.h"
#include "TableContainerManager.h"
#include "ItemManager.h"
#include "ItemDrop.h"


CWinterEvent::CWinterEvent()
{
	Init();
}

CWinterEvent::~CWinterEvent()
{
}

void CWinterEvent::Init()
{
	m_bOn = false;
	m_timeStart = 0;
	m_timeEnd = 0;
	m_dwNextUpdateTick = 10000;
	m_nMonsterSummoned = 0;
}


void CWinterEvent::StartEvent()
{
	if (m_bOn)
		return;

	CGameServer* app = (CGameServer*)g_pApp;

	CNtlStringW msg;

	CNtlPacket packet(sizeof(sGU_SYSTEM_DISPLAY_TEXT));
	sGU_SYSTEM_DISPLAY_TEXT * res = (sGU_SYSTEM_DISPLAY_TEXT*)packet.GetPacketData();
	res->wOpCode = GU_SYSTEM_DISPLAY_TEXT;
	res->wMessageLengthInUnicode = (WORD)msg.Format(L"Winter Event Started. Kill monsters to summon event monsters.");
	res->byDisplayType = SERVER_TEXT_EMERGENCY;
	NTL_SAFE_WCSCPY(res->awchMessage, msg.c_str());
	packet.SetPacketLen(sizeof(sGU_SYSTEM_DISPLAY_TEXT));
	g_pObjectManager->SendPacketToAll(&packet);
}

void CWinterEvent::TickProcess(DWORD dwTick)
{
	m_dwNextUpdateTick = UnsignedSafeDecrease<DWORD>(m_dwNextUpdateTick, dwTick);
	if (m_dwNextUpdateTick > 0)
		return;

	CGameServer* app = (CGameServer*)g_pApp;

	DBOTIME curTime = app->GetTime();

	tm timeStruct = {}; // int tm_mday;  // day of the month - [1, 31]            int tm_mon;   // months since January - [0, 11]
	localtime_s(&timeStruct, &curTime);

	enum months {
		JANUARY = 0,
		FEBRUARY,
		MARCH,
		APRIL,
		MAY,
		JUNE,
		JULY,
		AUGUST,
		SEPTEMBER,
		OCTOBER,
		NOVEMBER,
		DECEMBER
	};

	if (m_bOn == false)
	{
		//if ((timeStruct.tm_mon >= MARCH && timeStruct.tm_mday >= 16) && (timeStruct.tm_mon <= MAY && timeStruct.tm_mday <= 15))
		if ((timeStruct.tm_mon >= JANUARY && timeStruct.tm_mday >= 16) && (timeStruct.tm_mon <= MAY && timeStruct.tm_mday <= 15))
		{
			m_bOn = true;

			StartEvent();
		}
	}
	else
	{
		if (timeStruct.tm_mon == MAY && timeStruct.tm_mday == 16)
		{
			EndEvent();
		}
	}

	m_dwNextUpdateTick = 300000; //update again in 5 minutes
}

void CWinterEvent::Update(CMonster * pMob, CCharacter * pPlayer)
{
	if (m_bOn == false)
		return;

	if (pPlayer->GetCurWorld() == NULL)
		return;

	CItemDrop* pDrop = NULL;
	if (pMob->GetTblidx() == 2741101)
		pDrop = g_pItemManager->CreateSingleDrop(70.f, 11160033);
	if (pMob->GetTblidx() == 2741102)
		pDrop = g_pItemManager->CreateSingleDrop(70.f, 11160034);
	if (pMob->GetTblidx() == 7441101)
		pDrop = g_pItemManager->CreateSingleDrop(70.f, 11160035);

	if (pDrop)
	{
		sVECTOR3 pos;
		pos.x = pMob->GetCurLoc().x + RandomRangeF(-2.0f, 2.0f);
		pos.y = pMob->GetCurLoc().y;
		pos.z = pMob->GetCurLoc().z + RandomRangeF(-2.0f, 2.0f);

		pDrop->SetNeedToIdentify(false);
		pDrop->SetOwnership(pPlayer->GetID(), pPlayer->GetPartyID());
		pDrop->StartDestroyEvent();
		pDrop->AddToGround(pMob->GetWorldID(), pos);

		return;
	}

	if (Dbo_CheckProbability(5) == false)
		return;

	sMOB_TBLDAT* pTbldat = NULL;
	if (pMob->GetTbldat()->byGrade == MOB_GRADE_NORMAL)
		pTbldat = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(2741101);
	if (pMob->GetTbldat()->byGrade == MOB_GRADE_SUPER || pMob->GetTbldat()->byGrade == MOB_GRADE_ULTRA)
		pTbldat = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(2741102);
	if (pMob->GetTbldat()->byGrade == MOB_GRADE_BOSS || pMob->GetTbldat()->byGrade == MOB_GRADE_HERO)
		pTbldat = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(7441101);

	if (pTbldat)
	{
		sMOB_DATA data;
		InitMobData(data);

		data.worldID = pMob->GetWorldID();
		data.worldtblidx = pMob->GetWorldTblidx();

		data.tblidx = pTbldat->tblidx;

		pMob->GetCurLoc().CopyTo(data.vCurLoc);
		pMob->GetCurLoc().CopyTo(data.vSpawnLoc);
		pMob->GetCurDir().CopyTo(data.vCurDir);
		pMob->GetCurDir().CopyTo(data.vSpawnDir);

		data.actionpatternTblIdx = 1;

		CMonster* pNewMob = (CMonster*)g_pObjectManager->CreateCharacter(OBJTYPE_MOB);
		if (pNewMob)
		{
			if (pNewMob->CreateDataAndSpawn(data, pTbldat))
			{
				pNewMob->SetStandAlone(true);
			}
		}
	}
}

void CWinterEvent::EndEvent()
{
	if (m_bOn == false)
		return;

	Init();

	CNtlStringW msg;

	CNtlPacket packet(sizeof(sGU_SYSTEM_DISPLAY_TEXT));
	sGU_SYSTEM_DISPLAY_TEXT * res = (sGU_SYSTEM_DISPLAY_TEXT*)packet.GetPacketData();
	res->wOpCode = GU_SYSTEM_DISPLAY_TEXT;
	res->wMessageLengthInUnicode = (WORD)msg.Format(L"Winter Event Ended");
	res->byDisplayType = SERVER_TEXT_EMERGENCY;
	NTL_SAFE_WCSCPY(res->awchMessage, msg.c_str());
	packet.SetPacketLen(sizeof(sGU_SYSTEM_DISPLAY_TEXT));
	g_pObjectManager->SendPacketToAll(&packet);
}

void CWinterEvent::LoadEvent(HSESSION hSession)
{
	if (m_bOn == false)
		return;

	CNtlStringW msg;

	CNtlPacket packetMsg(sizeof(sGU_SYSTEM_DISPLAY_TEXT));
	sGU_SYSTEM_DISPLAY_TEXT * resMsg = (sGU_SYSTEM_DISPLAY_TEXT *)packetMsg.GetPacketData();
	resMsg->wOpCode = GU_SYSTEM_DISPLAY_TEXT;
	resMsg->byDisplayType = SERVER_TEXT_EMERGENCY;
	resMsg->wMessageLengthInUnicode = (WORD)msg.Format(L"Winter Event is currently running!");
	NTL_SAFE_WCSCPY(resMsg->awchMessage, msg.c_str());
	g_pApp->Send(hSession, &packetMsg);
}
