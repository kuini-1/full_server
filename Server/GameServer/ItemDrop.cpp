...
void CItemDrop::GenerateOptionSet(sITEM_TBLDAT* table, bool bEnchantAble)
{
	...
	if (bEnchantAble)
	{
		if (m_byRank < ITEM_RANK_SUPERIOR || m_byRank > ITEM_RANK_LEGENDARY)
			return;
		if (table->byItem_Type > ITEM_TYPE_RING)
			return;
		...
		for (int i = 0; i < m_byRank - 1; i++)
		{
			//	printf("%u | item worth %f GetRank() %u %u\n", GetTblidx(), fRemainingWorth, GetRank(), m_pProto.byRank);
			if (!g_pTableContainer->GetItemEnchantTable()->GenRandomOption(table, m_byRank, i == 0, i == m_byRank - 1, &flag, &fRemainingWorth, m_sOptionSet->aRandomOption[i], 0, 0, 0))
				break;
		}
	}
	else
