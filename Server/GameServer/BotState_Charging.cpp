...
void CBotState_Charging::OnEnter()
{
	...
	m_fDestValue = (float)m_pParent->GetCharAtt()->GetLastMaxRP();
	m_fRPChargingTime = (m_fDestValue - m_fCurrentValue) / (m_pParent->GetCharAtt()->GetLastRPRegen() / 1);
...
int CBotState_Charging::OnUpdate(DWORD dwTickDiff, float fMultiple)
{
	...
	if ((WORD)fValue >= m_pParent->GetCharAtt()->GetLastMaxRP())
	{
		if (m_pParent->GetCurRPBall() < m_pParent->GetMaxRPBall())
		{
			...
			m_fRPChargingTime = m_fDestValue / (m_pParent->GetCharAtt()->GetLastRPRegen() / 1);
