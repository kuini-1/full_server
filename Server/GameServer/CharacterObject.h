...
public:
	...
	inline bool			IsSliding() { return GetStateManager()->IsCharState(CHARSTATE_SLIDING); }
	...
	void			SetSettingMaxLP(float nSetting) { GetCharAtt()->SetSettingMaxLP(nSetting); }
	void			SetSettingPhysicalDefence(float nSetting) { GetCharAtt()->SetSettingPhysicalDefence(nSetting); }
	void			SetSettingEnergyDefence(float nSetting) { GetCharAtt()->SetSettingEnergyDefence(nSetting); }
	void			SetSettingPhysicalOffence(float nSetting) { GetCharAtt()->SetSettingPhysicalOffence(nSetting); }
	void			SetSettingEnergyOffence(float nSetting) { GetCharAtt()->SetSettingEnergyOffence(nSetting); }
	...
	void			SetLastMaxLP(int nMaxLp)	{ GetCharAtt()->SetLastMaxLP(nMaxLp); }
	int				GetLastMaxLP()				{ return GetCharAtt()->GetLastMaxLP(); }
	void			SetLastMaxEP(WORD wMaxEP)	{ GetCharAtt()->SetLastMaxEP(wMaxEP); }
	WORD			GetLastMaxEP()				{ return GetCharAtt()->GetLastMaxEP(); }

	inline float	GetCurLpInPercent()	{ return ((float)m_curLP / (float)GetCharAtt()->GetLastMaxLP()) * 100.0f; }
	inline float	GetCurEpInPercent()	{ return ((float)m_wCurEP / (float)GetCharAtt()->GetLastMaxEP()) * 100.0f; }
	inline float	GetCurApInPercent() { return ((float)m_curAP / (float)GetCharAtt()->GetLastMaxAP()) * 100.0f; }

	virtual float	GetWalkingSpeed()				{ return GetCharAtt()->GetLastWalkSpeed(); }
	void			SetLastRunSpeed(float speed)	{ GetCharAtt()->SetLastRunSpeed(speed); }
	virtual float	GetLastRunSpeed()				{ return GetCharAtt()->GetLastRunSpeed(); }
	void			SetLastFlySpeed(float speed)	{ GetCharAtt()->SetLastFlySpeed(speed); }
	float			GetLastFlySpeed()				{ return GetCharAtt()->GetLastRunSpeed(); }
	void			SetLastFlyDashSpeed(float speed){ GetCharAtt()->SetLastFlyDashSpeed(speed); }
	float			GetLastFlyDashSpeed()			{ return GetCharAtt()->GetLastFlyDashSpeed(); }
	void			SetLastFlyAccelSpeed(float speed){ GetCharAtt()->SetLastFlyAccelSpeed(speed); }
	float			GetLastFlyAccelSpeed()			{ return GetCharAtt()->GetLastFlyAccelSpeed(); }

	void			SetLastAttackSpeedRate(WORD speed){ GetCharAtt()->SetLastAttackSpeedRate(speed); }
	WORD			GetLastAttackSpeedRate()		  { return GetCharAtt()->GetLastAttackSpeedRate(); }

	void			SetLastSkillAnimationSpeedModifier(float speed){ GetCharAtt()->SetLastSkillAnimationSpeedModifier(speed); }
	float			GetLastSkillAnimationSpeedModifier()		  { return GetCharAtt()->GetLastSkillAnimationSpeedModifier(); }
