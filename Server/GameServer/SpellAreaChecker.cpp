...
bool CSpellAreaChecker::IsObjectInApplyRange(CSpawnObject *pObject, float *pfSquaredLength)
{
	if (pObject)
	{
		switch (m_byApplyRangeType)
		{
			...
			{
				CNtlVector vTemp(m_vAppointTargetLoc.operator-(pObject->GetCurLoc()));
				vTemp.y = 0.0f;
...
bool CSpellAreaChecker::IsObjectInCircleRange(CSpawnObject* pObject)
{
	
	if (m_byApplyRangeType == DBO_SKILL_APPLY_RANGE_CIRCLE)
	{
		if (m_hCaster->IsOutRange(pObject->GetCurLoc(), (float)sqrt((m_fRadius - (m_hCaster->GetObjectRadius() + pObject->GetObjectRadius())) / 3.14) * 2 * (m_hCaster->GetObjectRadius() - pObject->GetObjectRadius()) - (m_hCaster->GetObjectRadius() + pObject->GetObjectRadius())))
			return false;
		else
			return true;
	}
