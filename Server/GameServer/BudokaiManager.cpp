...
void CBudokaiManager::TickProcess(DWORD dwTickDiff)
{
	...
	{
		/*
		if (timeStruct.tm_wday == SATURDAY && timeStruct.tm_hour == 4)
		{
		...
		else if (timeStruct.tm_wday == THURSDAY && timeStruct.tm_hour == 9)
		{
			//start adult solo budokai
			m_bAdultBudokaiBegan = true;
	...
	{
		/*
		if (timeStruct.tm_wday == TUESDAY && timeStruct.tm_hour == 0)
		{
	...
	{
		/*
		if (timeStruct.tm_wday == SUNDAY && timeStruct.tm_hour == 23)
		{
	...
	{
		/*
		if (timeStruct.tm_wday == FRIDAY && timeStruct.tm_hour == 3)
		{
...
void CBudokaiManager::CloseBudokai()
{
	...
	for (std::unordered_map<JOINID, sBUDOKAI_REGISTER_INDIVIDUAL_DATA>::iterator it = m_mapIndividual.begin(); it != m_mapIndividual.end(); it++)
	{
	...
	for (std::unordered_map<JOINID, sBUDOKAI_REGISTER_TEAM_DATA>::iterator it = m_mapTeam.begin(); it != m_mapTeam.end(); it++)
	{
	...
	switch (m_matchStateInfo[m_matchType].byState)
	{
		...
		case BUDOKAI_MATCHSTATE_WAIT_MINOR_MATCH:
		{
			if (m_matchStateInfo[m_matchType].tmNextStepTime <= curTime)
			{
				...
				//start prelim
				/*
				if ((int)m_mapIndividual.size() >= 5) // by kuini must have 5+ players to start budokai
				{
					SendMatchState();
					StartPrelim();
				}
...
void CBudokaiManager::StartPrelim()
{
	...
	if (m_matchType == BUDOKAI_MATCH_TYPE_INDIVIDIAUL)
	{
		...
		//	printf("it->first: %u, charId:%u, fPoint %f \n", it->first, it->second.charId, it->second.fPoint);
			...
			{
				std::unordered_map<JOINID, sBUDOKAI_REGISTER_INDIVIDUAL_DATA>::iterator realIt = m_mapIndividual.find(it->first);
...
void CBudokaiManager::SendMinorMatchTeamInfo(BYTE byMatchIndex, sPRELIM_HEAD* prelim)
{
	...
	if (m_matchType == BUDOKAI_MATCH_TYPE_INDIVIDIAUL)
	{
		...
		for (std::map<HOBJECT, sPLAYER_INFO>::iterator it = prelim->m_mapIndividualPrelim.begin(); it != prelim->m_mapIndividualPrelim.end(); it++)
		{
			...
			{
				std::unordered_map<JOINID, sBUDOKAI_REGISTER_INDIVIDUAL_DATA>::iterator it2 = m_mapIndividual.find(pPlayer->GetJoinID());
...
void CBudokaiManager::DecidePrelimWinner(sPRELIM_HEAD* prelim, BYTE byMatchIndex)
{
	...
	else if (m_matchType == BUDOKAI_MATCH_TYPE_TEAM)
	{
		...
		// -- Check if there are teams with same amount of kills
		auto x = std::max_element(mapKills.begin(), mapKills.end(),
...
void CBudokaiManager::MinorMatchMatchFinish(sPRELIM_HEAD* prelim, BYTE byMatchIndex)
{
	...
	if (m_matchType == BUDOKAI_MATCH_TYPE_INDIVIDIAUL)
	{
		std::unordered_map<JOINID, sBUDOKAI_REGISTER_INDIVIDUAL_DATA>::iterator it2 = m_mapIndividual.find(prelim->wPrelimWinner);
		if (it2 != m_mapIndividual.end())
		{
	...
	{
		std::unordered_map<JOINID, sBUDOKAI_REGISTER_TEAM_DATA>::iterator it2 = m_mapTeam.find(prelim->wPrelimWinner);
...
void CBudokaiManager::MinorMatchSpawnPlayerRandomPlace(CPlayer * pPlayer, bool bSpawnFromFaint)
{
	if (pPlayer->GetCurWorld())
	{
		...
		pPlayer->UpdateCurLP(pPlayer->GetLastMaxLP(), true, false);
		pPlayer->UpdateCurEP(pPlayer->GetLastMaxEP(), true, false);
		pPlayer->UpdateRpBall(pPlayer->GetMaxRPBall(), false, false);
		pPlayer->UpdateCurRP(pPlayer->GetCharAtt()->GetLastMaxRP(), false, false);
	...
	if (m_matchType == BUDOKAI_MATCH_TYPE_INDIVIDIAUL)
	{
		for (std::map<HOBJECT, sPLAYER_INFO>::iterator it = match->m_mapIndividual.begin(); it != match->m_mapIndividual.end(); it++)
		{
			...
			if (pPlayer && pPlayer->GetCharID() == it->second.charId)
			{
				...
				{
					//pPlayer->AddCharTitle(titleId - 1); by kuini disabled budokai title
				}
			}
		}
	}
...
void CBudokaiManager::LoadBudokaiStateInfo(CPlayer * pPlayer)
{
	...
	if (m_matchType == BUDOKAI_MATCH_TYPE_INDIVIDIAUL)
	{
		...
		{
			std::unordered_map<JOINID, sBUDOKAI_REGISTER_INDIVIDUAL_DATA>::iterator it3 = m_mapIndividual.find(pPlayer->GetJoinID());
	...
	else if (m_matchType == BUDOKAI_MATCH_TYPE_TEAM)
	{
		if (m_matchStateInfo[m_matchType].byState == BUDOKAI_MATCHSTATE_MINOR_MATCH)
		{
			std::unordered_map<JOINID, sBUDOKAI_REGISTER_TEAM_DATA>::iterator it3 = m_mapTeam.find(pPlayer->GetJoinID());
...
void CBudokaiManager::JoinTeamReq(CPlayer * pPlayer, WCHAR * wszTeamName)
{
	...
	{
		size_t len = WCHARLen(wszTeamName);
		wchar_t* wbuf = new wchar_t[len + 1];
		for (size_t i = 0; i <= len; i++)
			wbuf[i] = (wchar_t)wszTeamName[i];
		wstrTeamName = std::wstring(wbuf);
		delete[] wbuf;
	}
...
void CBudokaiManager::LeaveTeamReq(CPlayer * pPlayer)
{
	...
	if (wResultcode == GAME_SUCCESS)
	{
		std::unordered_map<JOINID, sBUDOKAI_REGISTER_TEAM_DATA>::iterator it = m_mapTeam.find(pPlayer->GetJoinID());
		if (it != m_mapTeam.end())
		{
...
{
	std::unordered_map<JOINID, sBUDOKAI_REGISTER_INDIVIDUAL_DATA>::iterator it = m_mapIndividual.find(joinid);
...
{
	std::unordered_map<JOINID, sBUDOKAI_REGISTER_TEAM_DATA>::iterator it = m_mapTeam.find(joinid);
	if (it != m_mapTeam.end())
	{
		m_mapJoinInfo.erase(joinid);
