...
public:
	...
	void			SendCharStateKeepingEffect();
	...
	bool			SendCharStateFollowing(HOBJECT hTarget, float fDistance, BYTE byMoveReason, sVECTOR3& vDestLoc, bool bRunMode);
	...
	void			SendCharStateAirDashAccel(BYTE byMoveDir);
	bool			SendCharStateMoving(BYTE byMoveDir, BYTE byFlag, BYTE byStatus);
...
public:
	...
	virtual float		GetAttackRange() { return GetCharAtt()->GetLastAttackRange(); }
...
public:
	...
	void				InsertKeepEffectTarget(HOBJECT hObj, TBLIDX skillIdx);
	void				EraseKeepEffectTarget(HOBJECT hObj);
	void				RemoveKeepEffectTargets();
	bool				HasKeepEffectTargets() { return m_mapKeepEffectTargets.size() > 0; }
...
private:
	...
	std::multimap<HOBJECT, TBLIDX>	m_mapKeepEffectTargets; //<OBJ-Handle, Skill-ID>
...
public:
	...
	BYTE			GetSizeRate() { return m_bySizeRate; }
