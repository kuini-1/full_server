...
void CSkill::TickProcess(DWORD dwTickDiff)
{
	...
	{
		CMonster * Monster = g_pObjectManager->GetMob(m_pOwnerRef->GetID());
		for (int i = 0; i < m_byApplyTargetCount; i++)
		{
			CPlayer * Pplayer = g_pObjectManager->GetPC(m_ahApplyTarget[i]);
			if (Monster == NULL || Pplayer == NULL)
				return;
			
				float fDistance = NtlGetDistance(Monster->GetCurLoc(), Pplayer->GetCurLoc());
			            
				if (fDistance >= m_pSkillDataRef->byApply_Area_Size_1)
				{
		...
		*/
		...
		for (int i = 0; i < m_byApplyTargetCount; i++)
		{
			...
			if (pTarget)
			{
				...
				if (res->bIsSkillHarmful)
				{
					...
					if (m_pSkillDataRef->bySkill_Active_Type == SKILL_ACTIVE_TYPE_DC || m_pSkillDataRef->bySkill_Active_Type == SKILL_ACTIVE_TYPE_CB)
					{
						...
						//dwKeepTime = UnsignedSafeDecrease<DWORD>(dwKeepTime, DWORD((float)dwKeepTime * ((float)pTarget->GetCharAtt()->GetLastFoc() * 0.05f) / 100.f));

						float fDecreaseRate = float(pTarget->GetCharAtt()->GetLastFoc() - pTarget->GetCharAtt()->GetBaseFoc()) * 0.20f;
						float fDecreaseRateLimit = GetCcFocReductionLimit(m_aeEffectCode); // get limit

						if (fDecreaseRate > fDecreaseRateLimit)
							fDecreaseRate = fDecreaseRateLimit;
						else if (fDecreaseRate <= 0.0f)
							fDecreaseRate = 0.0f;

						dwKeepTime = UnsignedSafeDecrease<DWORD>(dwKeepTime, DWORD((float)dwKeepTime * fDecreaseRate / 100.f));
					}
					...
					{
						bool bCanTargetGetSliding = pTarget->GetStateManager()->CanCharStateTransition(CHARSTATE_SLIDING);
						if (bCanTargetGetSliding)
						{
							float fSlidingPercent = float(pTarget->GetCharAtt()->GetLastBlockRate() - pTarget->GetCharAtt()->GetBaseBlockRate()) * 0.021f;
						//	NTL_PRINT(PRINT_APP, "fSlidingPercent: %f", fSlidingPercent);
							if (fSlidingPercent > 35.f)
								fSlidingPercent = 35.f;

							
							if (Dbo_CheckProbabilityF(fSlidingPercent))
							{
								sCHAR_DATA_INFO* pCharInfo = m_pOwnerRef->GetAniTbldat()->GetAnim(0, GetOriginalTableData()->wAction_Animation_Index);

								if(pCharInfo->bKB2Push == false)
									res->aSkillResult[i].byAttackResult = BATTLE_ATTACK_RESULT_SLIDING;
							}
						}
					}

					/* - CHECK CRIT - */
					if (res->aSkillResult[i].byAttackResult != BATTLE_ATTACK_RESULT_KNOCKDOWN && res->aSkillResult[i].byAttackResult != BATTLE_ATTACK_RESULT_SLIDING)
					{
				...
				for (int e = 0; e < NTL_MAX_EFFECT_IN_SKILL; e++)
				{
					...
					//	NTL_PRINT(PRINT_APP,"Effect nr %u | effect %s", e, NtlGetSystemEffectString(effectCode));
						...
						case ACTIVE_BACKSTAB:
						{
							...
							{
								if (m_pOwnerRef->IsPC() && ((CPlayer*)m_pOwnerRef)->GetClass() == PC_CLASS_SWORD_MASTER)
									fSkillDmg *= res->aSkillResult[i].byAttackResult != BATTLE_ATTACK_RESULT_CRITICAL_HIT ? NTL_BATTLE_HIT_BACK_SIDE_BONUS_RATE : (NTL_BATTLE_HIT_BACK_SIDE_BONUS_RATE + 0.25f);
								else
						...
						case ACTIVE_EXPLOSION_ALL_DOT:
						{
								...
								|| pTarget->GetStateManager()->IsCharCondition(CHARCOND_STOMACHACHE) || pTarget->GetStateManager()->IsCharCondition(CHARCOND_BURN))
							{
								...
								if (res->aSkillResult[i].byAttackResult == BATTLE_ATTACK_RESULT_CRITICAL_HIT)
								{
									...
									fDOTDMG += fDOTDMG * fCritDmgRate / 100;
								}
						...
						case ACTIVE_BLEED://DOT buff effect // Skills with DOT effect never have 2x DOT effects in a skill. Its mostly 1x DOT and 1x Deal DMG.
						{
							...
							*/
							...
							//calculate the damage
							CalcSkillDotDamage(m_pOwnerRef, pTarget, m_pSkillDataRef, e, pTarget->GetCharAtt()->GetLastBleedDefence(), (float)m_pSkillDataRef->aSkill_Effect_Value[e],
							...
							// crane passive
							aBuffParameter[e].buffParameter.DOT_fDamage += aBuffParameter[e].buffParameter.DOT_fDamage * m_pOwnerRef->GetCharAtt()->GetLastDotValueChangePercent() / 100.f;
						...
						case ACTIVE_POISON://DOT buff effect // Skills with DOT effect never have 2x DOT effects in a skill. Its mostly 1x DOT and 1x Deal DMG.
						{
							...
							*/
							...
							//calculate the damage
							CalcSkillDotDamage(m_pOwnerRef, pTarget, m_pSkillDataRef, e, pTarget->GetCharAtt()->GetLastPoisonDefence(), (float)m_pSkillDataRef->aSkill_Effect_Value[e],
						...
						case ACTIVE_STOMACHACHE://DOT buff effect // Skills with DOT effect never have 2x DOT effects in a skill. Its mostly 1x DOT and 1x Deal DMG.
						{
							...
							*/
							...
							//calculate the damage
							CalcSkillDotDamage(m_pOwnerRef, pTarget, m_pSkillDataRef, e, pTarget->GetCharAtt()->GetLastStomachacheDefence(), (float)m_pSkillDataRef->aSkill_Effect_Value[e],
						...
						case ACTIVE_BURN://DOT buff effect // Skills with DOT effect never have 2x DOT effects in a skill. Its mostly 1x DOT and 1x Deal DMG.
						{
							...
							*/
							...
							//calculate the damage
							CalcSkillDotDamage(m_pOwnerRef, pTarget, m_pSkillDataRef, e, pTarget->GetCharAtt()->GetLastBurnDefence(), (float)m_pSkillDataRef->aSkill_Effect_Value[e],
						...
						case ACTIVE_LP_STEAL:
						{
							...
							else if (m_pSkillDataRef->bySkill_Effect_Type[e] == SYSTEM_EFFECT_APPLY_TYPE_PERCENT) //used for dragons punishment(dark warrior skill)
							{
								...
								if (m_pOwnerRef->IsPC() && m_pSkillDataRef->byRequire_Epuip_Slot_Type == EQUIP_SLOT_TYPE_SUB_WEAPON)
									...
									fTmpState = (float)(m_pOwnerRef->GetCharAtt()->GetLastPhysicalOffence() + m_pOwnerRef->GetCharAtt()->GetLastEnergyOffence() / 2);
						...
						case ACTIVE_STONE:
						{
							...
							else if (Dbo_CheckProbability((int)pTarget->GetCharAtt()->GetLastStoneToleranceRate()))
							{
								res->aSkillResult[i].byAttackResult = BATTLE_ATTACK_RESULT_RESISTED;
								goto RESULT2;
							}
						...
						case ACTIVE_CANDY:
						{
							...
							else if (Dbo_CheckProbability((int)pTarget->GetCharAtt()->GetLastCandyToleranceRate()))
							{
								res->aSkillResult[i].byAttackResult = BATTLE_ATTACK_RESULT_RESISTED;
								goto RESULT2;
							}
						...
						default: //to avoid having a case for every effect we just use the default
						{
							...
							{
								fEffectVal += (float)m_pSkillDataRef->aSkill_Effect_Value[e] * m_pOwnerRef->GetCharAtt()->GetLastDotValueChangePercent() / 100.f;
					...
					if (res->aSkillResult[i].byAttackResult == BATTLE_ATTACK_RESULT_BLOCK && res->bIsSkillHarmful && bCanCounterAttack)
					{
						...
						if (pCountAttack && m_pOwnerRef->IsInRange(pTarget, pCountAttack->GetOriginalTableData()->fUse_Range_Max))
						{
							if (Dbo_CheckProbability(50)) // chance to use passive "counter attack"
							{
								...
								pCountAttack->UseSkill(INVALID_BYTE, ahTempCounterAttack, m_pOwnerRef->GetCurLoc(), 1, &ahTempCounterAttack, wTempCounterAttackRes);
							}
						}
					}
			...
			RESULT3:
				...
				if (res->aSkillResult[i].byAttackResult == BATTLE_ATTACK_RESULT_KNOCKDOWN && m_pSkillDataRef->fUse_Range_Max <= 3.0f)
				{
					CNtlVector vShift(pTarget->GetCurLoc() - m_pOwnerRef->GetCurLoc());
					vShift.y = 0.0f;
					vShift.SafeNormalize();
					vShift.CopyTo(res->aSkillResult[i].vShift);
					pTarget->SendCharStateKnockdown(res->aSkillResult[i].vShift);
				}
		...
		{
			// we have to send "SendCharStateKeepingEffect" before we send the skill_result packet. Otherwise we have bad animation on skills like "Play Death"
...
void CSkill::OnAffected()
{
	...
	for (int i = 0; i < m_actionSkill.bySkillResultCount; i++)
	{
		...
		if (pTarget && pTarget->IsInitialized() && pTarget->IsFainting() == false)
		{
			...
			if (m_actionSkill.aSkillResult[i].byAttackResult == BATTLE_ATTACK_RESULT_KNOCKDOWN && pTarget->GetCharStateID() != CHARSTATE_KNOCKDOWN)
			{
				CNtlVector vShift(pTarget->GetCurLoc() - m_pOwnerRef->GetCurLoc());
				vShift.y = 0.0f;
				vShift.SafeNormalize();
			...
			else if (m_actionSkill.aSkillResult[i].byAttackResult == BATTLE_ATTACK_RESULT_SLIDING && pTarget->GetCharStateID() != CHARSTATE_SLIDING)
			{
				CNtlVector vShift(pTarget->GetCurLoc() - m_pOwnerRef->GetCurLoc());
				vShift.y = 0.0f;
				vShift.SafeNormalize();
				vShift.CopyTo(m_actionSkill.aSkillResult[i].vShift);
				pTarget->SendCharStateSliding(m_actionSkill.aSkillResult[i].vShift);
			}
			...
			if (m_actionSkill.bIsSkillHarmful)
			{
				...
				if (fDmg > 0.0f && pTarget->OnSkillAction(m_pOwnerRef, (int)fDmg, 0, m_actionSkill.aSkillResult[i].byAttackResult, m_actionSkill.bCanWakeUpTarget) == false && m_actionSkill.bCanTargetRecoverOnHit) //deal dmg and check if not dead
				{
	...
	for (int e = 0; e < NTL_MAX_EFFECT_IN_SKILL; e++)
	{
		switch (GetEffectCode(e))
		{
			...
			case ACTIVE_GREAT_NAMEK:
			{
				...
				int nOldMaxLP = m_pOwnerRef->GetLastMaxLP();
				WORD wOldMaxEP = m_pOwnerRef->GetLastMaxEP();

				((CPlayer*)m_pOwnerRef)->Transform(GetEffectCode(e), (TBLIDX)m_pSkillDataRef->aSkill_Effect_Value[e], true, GetSkillIndex());
				
				if (m_pOwnerRef->GetLastMaxLP() - nOldMaxLP > 0)
					m_pOwnerRef->UpdateCurLP(m_pOwnerRef->GetLastMaxLP() - nOldMaxLP, true, false);

				if (m_pOwnerRef->GetLastMaxEP() - wOldMaxEP > 0)
					m_pOwnerRef->UpdateCurEP(m_pOwnerRef->GetLastMaxEP() - wOldMaxEP, true, false);
			}
			break;
