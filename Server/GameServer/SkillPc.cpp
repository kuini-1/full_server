...
bool CSkillPc::UseSkill(BYTE byRpBonusType, HOBJECT hAppointTargetHandle, CNtlVector & rvFinalSubjectLoc, BYTE byApplyTargetCount, HOBJECT * ahApplyTarget, WORD & rwResultCode)
{
	if (CSkill::UseSkill(byRpBonusType, hAppointTargetHandle, rvFinalSubjectLoc, byApplyTargetCount, ahApplyTarget, rwResultCode))
	{
				...
				if (pTarget->IsPC() || pTarget->IsSummonPet() || (pTarget->IsNPC() && ((CNpc*)pTarget)->GetPcRelation() != RELATION_TYPE_ENEMY))
				{
					...
					else if (pTarget->IsPC())
					{
						...
						if (byWorldRuleType == GAMERULE_NORMAL)
						{
							...
							if (app->GetGsChannel() == 2) {
								if (m_pPlayerRef->GetPartyID() != pTargetPc->GetPartyID() && m_pPlayerRef->GetGuildID() != pTargetPc->GetGuildID())
									continue;
							}
						}
						else if (byWorldRuleType == GAMERULE_RANKBATTLE)
