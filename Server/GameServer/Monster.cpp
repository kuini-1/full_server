...
void CMonster::CopyToObjectInfo(sOBJECT_INFO * pObjectInfo, CHARACTERID playerCharID)
{
	...
	pObjectInfo->mobBrief.fLastFlyingAccelSpeed = GetLastFlyAccelSpeed();
	pObjectInfo->mobBrief.fLastFlyingDashSpeed = GetLastFlyDashSpeed();
	pObjectInfo->mobBrief.fLastFlyingSpeed = GetLastFlySpeed();
	pObjectInfo->mobBrief.fLastRunningSpeed = GetLastRunSpeed();
	pObjectInfo->mobBrief.fLastWalkingSpeed = GetWalkingSpeed();
	pObjectInfo->mobBrief.fSkillAnimationSpeedModifier = GetLastSkillAnimationSpeedModifier();
	pObjectInfo->mobBrief.maxLp = GetLastMaxLP();
	pObjectInfo->mobBrief.nicknameTblidx = INVALID_TBLIDX;
	pObjectInfo->mobBrief.tblidx = GetTblidx();
	pObjectInfo->mobBrief.wAttackSpeedRate = GetLastAttackSpeedRate();
	pObjectInfo->mobBrief.wCurEP = GetCurEP();
	pObjectInfo->mobBrief.wMaxEP = GetLastMaxEP();
}
...
bool CMonster::CreateDataAndSpawn(WORLDID worldId, sMOB_TBLDAT* mobTbldat, sSPAWN_TBLDAT* spawnTbldat, bool bSpawnOnServerStart/* = false*/, BYTE bySpawnFuncFlag/* = SPAWN_FUNC_FLAG_RESPAWN*/)
{
	...
	sTempState.sCharStateBase.bIsValidLoc = true;
...
void CMonster::Spawn(bool bSpawnOnServerStart)
{
	...
	//SetSettingMaxLP(tbldat->fSettingRate_LP);
	//SetSettingPhysicalDefence(tbldat->fSettingRate_PhyDefence);
	//SetSettingEnergyDefence(tbldat->fSettingRate_EngDefence);
	//SetSettingPhysicalOffence(tbldat->fSettingPhyOffenceRate);
	//SetSettingEnergyOffence(tbldat->fSettingEngOffenceRate);

	SetCurLP(tbldat->dwBasic_LP);// * tbldat->fSettingRate_LP);
	SetExp(tbldat->dwExp);
	SetZeni(tbldat->dwDrop_Zenny);
	SetCurEP(tbldat->wBasic_EP);
	SetLastRunSpeed(tbldat->fRun_Speed);
	...
	if (!bSpawnOnServerStart)
	{
		...
		if (g_pDragonballHuntEvent->IsOn() && GetTbldat()->bIsDragonBallDrop)
		{
			...
			{
				if (Dbo_CheckProbability(GetTbldat()->byGrade + 3))
				{
...
const BYTE CURRENT_EVENT = WINTER;
...
bool CMonster::Faint(CCharacterObject* pkKiller, eFAINT_REASON byReason)
{
	if (CCharacterObject::Faint(pkKiller, byReason))
	{
		...
		{
			TBLIDX mobId = this->m_tblidx;
			CPlayer* pKiller = (pkKiller->IsSummonPet() == false) ? (CPlayer*)pkKiller : ((CSummonPet*)pkKiller)->GetOwner();

			if (pKiller && pKiller->IsInitialized())
			{
				// Check if killer is in battle royale world
				if (pKiller->GetCurWorld() && pKiller->GetCurWorld()->GetRuleType() == GAMERULE_RAID && pKiller->GetCurWorld()->GetTbldat() && pKiller->GetCurWorld()->GetTbldat()->tblidx == 212000)
				{
					if (g_pBattleRoyaleEvent)
					{
						g_pBattleRoyaleEvent->OnMonsterKill(this, pKiller);
						// Skip normal rewards in battle royale - EXP is handled by BattleRoyaleEvent
						// But we still need to call CreateKillReward for quest updates, etc.
						// Actually, let's skip CreateKillReward entirely for BR
						return true;
					}
				}

				CGameServer* app = (CGameServer*)g_pApp;
				const sKILL_DEBUFF_CONFIG& debuff = app->GetKillDebuffConfig();
				if (debuff.bEnable && debuff.skillTblidx != INVALID_TBLIDX)
				{
					ApplySkillBuffToCharacter(pKiller, debuff.skillTblidx, debuff.durationSeconds, debuff.bOverrideValues, debuff.aEffectValues);
				}
			}
			...
			CreateKillReward(GetCurWorld()->GetRuleType() != GAMERULE_CCBATTLEDUNGEON || !g_pBattleRoyaleEvent);
			g_pDynamicFieldSystemEvent->Update(this, pKiller);
			g_pEventSystemEvent->Update(this, pKiller);
			//g_pHoneyBeeEvent->Update(this, pKiller);
			//g_pWinterEvent->Update(this, pKiller);

			int l_LevelGap = abs(pKiller->GetLevel() - GetLevel());
			if (l_LevelGap <= 15) pKiller->UpdateNetpyToken(pKiller->GetNetpyTokens() + 1, 1, true);

			if (pKiller->GetAccountID() == 813756 || pKiller->GetAccountID() == 814068) // by kuini for logging mob tblidx
			{
				printf("Mob: %u   |   %f;%f;%f \n", this->m_tblidx, this->GetCurLoc().x, this->GetCurLoc().y, this->GetCurLoc().z);
			}
			...
			m_faintBuffIndex = SKILL TBLIDX
			m_byFaintBuffApplyType = APPLY_TYPE
			m_fFaintBuffRange = FLOAT RANGE
			*/
			
			if (this->m_tblidx == 1411101) SetFaintBuff(2014, eMOB_FAINT_BUFF_SLAYER, 15.f);
			...
			const TBLIDX EVENT_REWARD = 11140345;

			RandomMobDrop(this->GetTbldat()->byGrade + 1, EVENT_REWARD, 1, this, pKiller, ANY_DROP);
				
			if (mobId == 13211206) RandomMobDrop(100, EVENT_REWARD, 20, this, pKiller, ANY_DROP);

			if (CURRENT_EVENT == HONEY_BEE)
			{
				TBLIDX item = INVALID_TBLIDX;
				if (this->GetTblidx() == 2741101) RandomMobDrop(100, 11160033, 1, this, pKiller, ANY_DROP);
				if (this->GetTblidx() == 2741102) RandomMobDrop(100, 11160034, 1, this, pKiller, ANY_DROP);
				if (this->GetTblidx() == 7441101) RandomMobDrop(100, 11160035, 1, this, pKiller, ANY_DROP);

				if (Dbo_CheckProbability(5))
				{
					sMOB_TBLDAT* pTbldat = NULL;
					if (this->GetTbldat()->byGrade == MOB_GRADE_NORMAL)
						pTbldat = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(2741101);
					if (this->GetTbldat()->byGrade == MOB_GRADE_SUPER || this->GetTbldat()->byGrade == MOB_GRADE_ULTRA)
						pTbldat = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(2741102);
					if (this->GetTbldat()->byGrade == MOB_GRADE_BOSS || this->GetTbldat()->byGrade == MOB_GRADE_HERO)
						pTbldat = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(7441101);
					...
					if (pTbldat)
					{
						...
						data.worldID = this->GetWorldID();
						data.worldtblidx = this->GetWorldTblidx();

						data.tblidx = pTbldat->tblidx;

						this->GetCurLoc().CopyTo(data.vCurLoc);
						this->GetCurLoc().CopyTo(data.vSpawnLoc);
						this->GetCurDir().CopyTo(data.vCurDir);
						this->GetCurDir().CopyTo(data.vSpawnDir);
			...
			//--------------------------------------------------------------------------------------//
			//	BOSS_DROP  ACCESSORY DUNGEON
			//--------------------------------------------------------------------------------------//

			if (mobId == 850002) RandomMobDrop(100, 850759, 1, this, pKiller, ANY_DROP);

			if (mobId == 850003 || mobId == 850002)
			{
				std::vector<TBLIDX> RECIPE { 850122, 850123, 850124, 850125, 850126, 850127, 850128, 850129, 850130 };
				std::vector<TBLIDX> MATERIAL{ 850131, 850132, 850133 };
				TBLIDX ITEM_DROP = INVALID_TBLIDX;

				if (pKiller->GetParty())
				{
					for (BYTE i = 0; i < pKiller->GetParty()->GetPartyMemberCount(); i++)
					{
						if (Dbo_CheckProbability(10)) ITEM_DROP = RECIPE[RandomRange(0, (int)RECIPE.size() - 1)];
						else ITEM_DROP = MATERIAL[RandomRange(0, (int)MATERIAL.size() - 1)];

						CPlayer* pPlayer = g_pObjectManager->GetPC(pKiller->GetParty()->GetMemberInfo(i).hHandle);
						RandomMobDrop(100, ITEM_DROP, 1, this, pPlayer, SELF_DROP);
					}
				}
			}
			...
			//--------------------------------------------------------------------------------------//
			//	BOSS_DROP  TMQ 6 HARD	
			//--------------------------------------------------------------------------------------//

			if (mobId == 31753416) // first boss
			{
				std::vector<TBLIDX> COSTUME{ 850122, 850123, 850124, 850125, 850126, 850127, 850128, 850129, 850130 };
				std::vector<TBLIDX> MATERIAL{ 850461, 850462, 850463, 850464, 850465, 850466 };
				TBLIDX ITEM_DROP = INVALID_TBLIDX;
				...
				if (pKiller->GetParty())
				{
					for (BYTE i = 0; i < pKiller->GetParty()->GetPartyMemberCount(); i++)
					{
						...
						if (Dbo_CheckProbability(2)) RandomMobDrop(100, COSTUME[RandomRange(0, (int)MATERIAL.size() - 1)], 1, this, pPlayer, SELF_DROP);
						ITEM_DROP = MATERIAL[RandomRange(0, (int)MATERIAL.size() - 1)];
			...
			if (mobId == 31753419) // second boss
			{
				std::vector<TBLIDX> RECIPE{ 850455, 850456, 850457, 850458, 850459, 850460 };
				std::vector<TBLIDX> MATERIAL{ 850461, 850462, 850463, 850464, 850465, 850466 };
				TBLIDX ITEM_DROP = INVALID_TBLIDX;
				...
				if (pKiller->GetParty())
				{
					for (BYTE i = 0; i < pKiller->GetParty()->GetPartyMemberCount(); i++)
					{
						if (Dbo_CheckProbability(2)) ITEM_DROP = RECIPE[RandomRange(0, (int)RECIPE.size() - 1)];
						else ITEM_DROP = MATERIAL[RandomRange(0, (int)MATERIAL.size() - 1)];
			...
			if (mobId == 33431101) // last boss
			{
				...
				if (pKiller->GetParty())
				{
					for (BYTE i = 0; i < pKiller->GetParty()->GetPartyMemberCount(); i++)
					{
						...
						CPlayer* pPlayer = g_pObjectManager->GetPC(pKiller->GetParty()->GetMemberInfo(i).hHandle);
						RandomMobDrop(100, ITEM_DROP, 1, this, pPlayer, SELF_DROP);
						RandomMobDrop(100, 850451, 1, this, pPlayer, SELF_DROP);
						RandomMobDrop(100, 850760, RandomRange(1, 3), this, pKiller, SELF_DROP);
						RandomMobDrop(100, 850761, RandomRange(1, 3), this, pKiller, SELF_DROP);
					}
				}
			}
		...
		/*
		CPlayer* pPlayer = (pkKiller->IsSummonPet() == false) ? (CPlayer*)pkKiller : ((CSummonPet*)pkKiller)->GetOwner();
		sMOB_TBLDAT* pMOBTblData = NULL; sVECTOR3 spawnloc;

		if (Dbo_CheckProbability(this->GetTbldat()->byGrade) && GetCurWorld()->GetRuleType() == GAMERULE_HUNT && (this->GetTbldat()->byLevel >= 20 && this->GetTbldat()->byLevel < 40))
			pMOBTblData = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(1760102);
		if (Dbo_CheckProbability(this->GetTbldat()->byGrade) && GetCurWorld()->GetRuleType() == GAMERULE_HUNT && (this->GetTbldat()->byLevel >= 40 && this->GetTbldat()->byLevel < 60))
			pMOBTblData = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(1755303);
		if (Dbo_CheckProbability(this->GetTbldat()->byGrade) && GetCurWorld()->GetRuleType() == GAMERULE_HUNT && (this->GetTbldat()->byLevel >= 60 && this->GetTbldat()->byLevel < 70))
			pMOBTblData = (sMOB_TBLDAT*)g_pTableContainer->GetMobTable()->FindData(1757107);
		if (pMOBTblData)
		{

			spawnloc.x = this->GetCurLoc().x + rand() % 5;
			spawnloc.y = 0.0f;
			spawnloc.z = this->GetCurLoc().z + rand() % 5;

			sVECTOR3 spawndir;
			spawndir.x = this->GetCurDir().x + rand() % 5;
			spawndir.y = 0.0f;
			spawndir.z = this->GetCurDir().z + rand() % 5;
			...
			sMobSpawn.bySpawn_Move_Type = SPAWN_MOVE_FIX;
			sMobSpawn.bySpawn_Loc_Range = 2;
		...
		std::vector<TBLIDX> items;
		if (this->GetTblidx() == 1760102)
		{
			if (Dbo_CheckProbability(24)) items.push_back(11120253);
			if (Dbo_CheckProbability(24)) items.push_back(403041);
			if (Dbo_CheckProbability(24)) items.push_back(19916);
			if (Dbo_CheckProbability(24)) items.push_back(11120221);
			if (Dbo_CheckProbability(2)) items.push_back(11140184);
			if (Dbo_CheckProbability(2)) items.push_back(11140327);
		}
		else if (this->GetTblidx() == 1755303)
		{
			if (Dbo_CheckProbability(24)) items.push_back(11120253);
			if (Dbo_CheckProbability(24)) items.push_back(403042);
			if (Dbo_CheckProbability(24)) items.push_back(19917);
			if (Dbo_CheckProbability(24)) items.push_back(11120220);
			if (Dbo_CheckProbability(2)) items.push_back(11120154);
			if (Dbo_CheckProbability(2)) items.push_back(403023);
		}
		else if (this->GetTblidx() == 1757107)
		{
			if (Dbo_CheckProbability(24)) items.push_back(11140184);
			if (Dbo_CheckProbability(24)) items.push_back(403043);
			if (Dbo_CheckProbability(24)) items.push_back(19918);
			if (Dbo_CheckProbability(24)) items.push_back(11120262);
			if (Dbo_CheckProbability(2)) items.push_back(11120093);
			if (Dbo_CheckProbability(2)) items.push_back(11120219);
		}

		TBLIDX ItemId = NULL;
		if (this->GetTblidx() == 1760102 || this->GetTblidx() == 1755303 || this->GetTblidx() == 1757107)
			ItemId = items[RandomRange(0, items.size() - 1)];

		CItemDrop* pBall = g_pItemManager->CreateSingleDrop(100.f, ItemId);
		if (pBall && pPlayer->GetCurWorld()->GetRuleType() == GAMERULE_HUNT && ItemId)
		{
			...
			vec.x += RandomRangeF(-5.0f, 5.0f);
			vec.z += RandomRangeF(-5.0f, 5.0f);

			pBall->SetNeedToIdentify(false);
			pBall->SetOwnership(pPlayer->GetID(), pPlayer->GetPartyID());
			pBall->StartDestroyEvent();
...
//	
//--------------------------------------------------------------------------------------//
void CMonster::RandomMobDrop(int prob, TBLIDX item, BYTE amount, CCharacterObject* mob, CPlayer* player, BYTE type)
{
	switch (type)
	{
	case SELF_DROP:
	{
		...
		{
			CItemDrop* pBall = g_pItemManager->CreateSingleDrop(prob, item);
			if (pBall)
			{
				CNtlVector vPos(mob->GetCurLoc());
				sVECTOR3 vec;
				...
				pBall->SetNeedToIdentify(false);
				pBall->SetOwnership(player->GetID(), INVALID_PARTYID);
				pBall->StartDestroyEvent();
				pBall->AddToGround(player->GetWorldID(), vec);
			}
		}
	} break;
	case ANY_DROP:
	{
		for (BYTE i = 0; i < amount; i++)
		{
			...
			if (pBall)
			{
				...
				pBall->SetNeedToIdentify(false);
				pBall->SetOwnership(player->GetID(), player->GetPartyID());
				pBall->StartDestroyEvent();
				pBall->AddToGround(player->GetWorldID(), vec);
			}
		}
		...
		for (BYTE i = 0; i < player->GetParty()->GetPartyMemberCount(); i++)
		{
			for (BYTE i = 0; i < amount; i++)
			{
				...
				if (pBall)
				{
					...
					pBall->SetNeedToIdentify(false);
					pBall->SetOwnership(player->GetParty()->GetMemberInfo(i).hHandle, INVALID_PARTYID);
					pBall->StartDestroyEvent();
					pBall->AddToGround(player->GetWorldID(), vec);
				}
			}
...
DWORD CalcExp(CMonster* victim, CPlayer* player, DWORD dw_Exp)
{
	...
	/*
	if (player->CheckCharTitle(11) || player->CheckCharTitle(12) || player->CheckCharTitle(13) || player->CheckCharTitle(14) || player->CheckCharTitle(15))
		dw_Exp = DWORD((float)dw_Exp * 0.2f);
	*/
...
typedef struct SDamageInfo
{
	...
	inline void Distribute(CMonster* ch, DWORD dwExp) //ch = monster
	{
		...
		else if (pParty)
		{
			...
			if(f.member_count > 1)	//only add party bonus if at least 1 member inside range
