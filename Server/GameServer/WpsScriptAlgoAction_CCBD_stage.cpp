...
//	NTL_PRINT(PRINT_APP, "enter stage %u \n", m_byStage);
	...
	{
		if (m_bIsEveryoneReady) TeleportToBoss();
	}
	else //if not boss stage
	{
		if (m_byStage > 1) //if not first stage then spawn at base loc
		{
			if(IsBossStage(m_byStage - 1) == false) //if previous stage was boss then do not spawn because we use teleport at OnExit
				SpawnNextStage();
		}
	}
}
...
{
	if (m_bIsEveryoneReady == false && IsBossStage(m_byStage)) //wait until everyone arrive in boss stage
	{
		m_dwWait = UnsignedSafeDecrease<DWORD>(m_dwWait, dwTickDiff);
		m_dwBossTimer = UnsignedSafeDecrease<DWORD>(m_dwBossTimer, dwTickDiff);

		if (IsEveryoneReady() && m_dwBossTimer == 0 && m_dwWait != 0 && m_bBoss == false) { m_bBoss = true; TeleportToBoss(); }
		else if (IsEveryoneReady() && m_dwWait == 0)
		{
			...
			res->byState = 0;
			res->dwLimitTime = INVALID_DWORD;
			res->byPatternIndex = GetOwner()->GetCCBDStage();
			res->titleTblidx = 41;
...
void CWpsScriptAlgoAction_CCBD_stage::TeleportToBoss()
{
	...
	BYTE byBossStageCount = m_byStage;
	WORLDID destWorld = GetOwner()->GetWorld()->GetID();
	CNtlVector destLoc(g_pTableContainer->GetServerConfigTable()->GetServerConfigData()->sBattleDungeonData.aEnterLoc_BossStage[byBossStageCount - 1].sLoc);
	CNtlVector destDir(g_pTableContainer->GetServerConfigTable()->GetServerConfigData()->sBattleDungeonData.aEnterLoc_BossStage[byBossStageCount - 1].sDir);
