...
bool CHtbSkillManager::RemoveSkillAll()
{
	while (!m_htbList.IsEmpty())
	{
		unsigned int pos = m_htbList.Begin();

		CHtbSkill* pSkill = m_htbList.GetAt(pos);
		if (pSkill)
		{
			SAFE_DELETE(pSkill);
		}

		m_htbList.Remove(pos);
	}

	for (DWORD j = 0; j < NTL_HTB_MAX_PC_HAVE_HTB_SKILL; j++)
		m_ppHtbSkill[j] = NULL;
...
void CHtbSkillManager::ResetAllCooldowns()
{
	for (unsigned int pos = m_htbList.Begin();
		m_htbList.End() != pos;
		pos = m_htbList.Next(pos))
