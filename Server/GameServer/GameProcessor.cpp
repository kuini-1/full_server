...
void CGameProcessor::Run(DWORD dwTickCount)
{
	...
	//	ERR_LOG(LOG_FIRST, "m_dwTickCount %u - m_dwLastTimeGameMainUpdated %u >= m_dwTickTime %u\n", m_dwTickCount, m_dwLastTimeGameMainUpdated, m_dwTickTime);
		...
		else
		{
			...
			g_pRankbattleManager->TickProcess(dwTickDiff);
			...
			g_pEventSystemEvent->TickProcess(dwTickDiff);
			...
			//g_pWinterEvent->TickProcess(m_dwTickCount);
			if (g_pBattleRoyaleEvent)
				g_pBattleRoyaleEvent->TickProcess(m_dwTickCount);
		}
...
void CGameProcessor::StartServerShutdownNotice(BYTE minutes)
{
	CNtlStringW msg;
	...
	DWORD tick = GetTickCount() + (minutes * 60000);
	bool shutdown = false;

	while (shutdown == false) 
	{
		printf("tick: %u, curTick: %u \n", tick, GetTickCount());
		if (tick < GetTickCount())
		{
			shutdown == true;

			StartServerShutdownEvent();
...
void CGameProcessor::StartServerShutdownEvent()
{
	...
	CNtlPacket packet2(sizeof(sGU_SHUTDOWN_COUNT_DOWN_START_NFY));
	sGU_SHUTDOWN_COUNT_DOWN_START_NFY* res2 = (sGU_SHUTDOWN_COUNT_DOWN_START_NFY*)packet2.GetPacketData();
	res2->wOpCode = GU_SHUTDOWN_COUNT_DOWN_START_NFY;
	res2->byMsgType = 1;
	packet2.SetPacketLen(sizeof(sGU_SHUTDOWN_COUNT_DOWN_START_NFY));
	g_pObjectManager->SendPacketToAll(&packet2);


	//Send packet to master server so no new people can connect.
	CGameServer* app = (CGameServer*)g_pApp;
	CNtlPacket packet3(sizeof(sGM_SERVER_SHUT_DOWN));
	sGM_SERVER_SHUT_DOWN* res3 = (sGM_SERVER_SHUT_DOWN*)packet3.GetPacketData();
	res3->wOpCode = GM_SERVER_SHUT_DOWN;
	packet3.SetPacketLen(sizeof(sGM_SERVER_SHUT_DOWN));
	app->SendTo(app->GetMasterServerSession(), &packet3);
