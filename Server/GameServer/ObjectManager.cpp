...
{
	for (std::unordered_map<CNpc*, DWORD>::iterator it = m_map_Respawn.begin(); it != m_map_Respawn.end(); it++)
	{
		CNpc* pBot = it->first;
		if (pBot)
		{
...
void CObjectManager::ProcessRespawn()
{
	if (m_map_Respawn.size() > 0)
	{
		...
		for (std::unordered_map<CNpc*, DWORD>::iterator it = m_map_Respawn.begin(); it != m_map_Respawn.end(); )
		{
			CNpc* pBot = it->first;
			if (pBot)
			{
...
void CObjectManager::ProcessDelayDeleteMap()
{
	...
	{
		DWORD dwCurTick = GetTickCount();
		for (std::unordered_map<CGameObject*, DWORD>::iterator it = m_map_DelayDelete.begin(); it != m_map_DelayDelete.end(); )
		{
