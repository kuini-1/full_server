			...
			switch (m_BattleState)
			{
				case RANKBATTLE_BATTLESTATE_DIRECTION: //used in the beginning when all players inside. Shows who fight vs who
				{
					...
					if (m_dwDirectionTime == 0)
					{
						...
						UpdateBattleStateNfy(RANKBATTLE_BATTLESTATE_STAGE_READY, 0);
					}
				}
				break;
				...
				break;
				case RANKBATTLE_BATTLESTATE_STAGE_RUN: //round X wait
				{
				...
				break;
				case RANKBATTLE_BATTLESTATE_STAGE_FINISH: // progress fight
				{
				...
				break;
				case RANKBATTLE_BATTLESTATE_MATCH_FINISH: //show round winner
				{
				...
				break;
				case RANKBATTLE_BATTLESTATE_BOSS_DIRECTION: //shows the winners etc
				{
...
{
	if (m_BattleState != RANKBATTLE_BATTLESTATE_STAGE_FINISH) // dont allow update state while match is not in progress
	{
		ERR_LOG(LOG_RANKBATTLE, "Could not update character %u state to %u because m_BattleState(%u) != RANKBATTLE_BATTLESTATE_STAGE_FINISH", pPlayer->GetCharID(), byState, m_BattleState);
		return;
	}
...
void CRankbattleBattle::DecideMatchWinner()
{
	...
	m_dwMatchFinishTime = m_pRankTblData->dwMatchFinishTime * 1000;
	UpdateBattleStateNfy(RANKBATTLE_BATTLESTATE_BOSS_DIRECTION, m_byStage);
}
...
void CRankbattleRoom::LeaveQueue(CPlayer * pPlayer)
{
	if (GetBattleMode() == RANKBATTLE_MODE_INDIVIDUAL)
	{
		std::unordered_map<HOBJECT, CHARACTERID>::iterator it = m_mapSoloQueue.find(pPlayer->GetID());
	...
	{
		for (std::unordered_map<HOBJECT, CParty*>::iterator it = m_mapPartyQueue.begin(); it != m_mapPartyQueue.end(); it++)
		{
...
void CRankbattleRoom::UpdateMatchmakingQueue()
{
	...
	if (GetBattleMode() == RANKBATTLE_MODE_INDIVIDUAL)
	{
		...
		for (std::unordered_map<HOBJECT, CHARACTERID>::iterator fit = vecTmpQueue.begin(); fit != vecTmpQueue.end(); )
		{
			bFound = false;
			...
			if (pFirst && pFirst->IsInitialized() && pFirst->GetCharID() == fit->second)
			{
				...
				for (std::unordered_map<HOBJECT, CHARACTERID>::iterator sit = m_mapSoloQueue.begin(); sit != m_mapSoloQueue.end(); sit++) //find a matching partner
				{
	...
	else
	{
		...
		for (std::unordered_map<HOBJECT, CParty*>::iterator fit = vecTmpQueue.begin(); fit != vecTmpQueue.end(); )
		{
			bFound = false;
			...
			if (pFirst && pFirst->GetPartyLeaderID() == fit->first && pFirst->GetPartyMemberCount() >= 2)
			{
				...
				for (std::unordered_map<HOBJECT, CParty*>::iterator sit = m_mapPartyQueue.begin(); sit != m_mapPartyQueue.end(); sit++) //find matching partner
				{
