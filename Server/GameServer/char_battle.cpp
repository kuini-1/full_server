...
void CCharacter::AttackAction(CCharacter* pVictim, bool bIsChainAttack)
{
	...
	//check dodge
	if (BattleIsDodge(pVictim->IsPC(), GetCharAtt()->GetLastAttackRate(), pVictim->GetCharAtt()->GetLastDodgeRate(), GetLevel(), pVictim->GetLevel()) == true)
	{
	...
	if (BattleIsBlock(pVictim->GetCharAtt()->GetLastBlockRate(), GetLevel(), pVictim->GetLevel()))
	{
		byAttackResult = BATTLE_ATTACK_RESULT_BLOCK;
		fDmg /= 2.f;
	}
	else
	{
		...
		if (byAttackType == BATTLE_ATTACK_TYPE_PHYSICAL)
		{
			...
			fCritDmgRate = GetCharAtt()->GetLastPhysicalCriticalDamageRate();
		}
		else if (byAttackType == BATTLE_ATTACK_TYPE_ENERGY)
		{
			...
			fCritDmgRate = GetCharAtt()->GetLastEnergyCriticalDamageRate();
		}

		if (bIsCrit)
		{
			byAttackResult = BATTLE_ATTACK_RESULT_CRITICAL_HIT;
			fDmg += fDmg * fCritDmgRate / 100.f;
		}
	}

	//set victim heal-on-hit
	fTargetLpRecoveredWhenHit = (float)pVictim->GetCharAtt()->GetLastLpRecoveryWhenHit() + (fDmg * pVictim->GetCharAtt()->GetLastLpRecoveryWhenHitInPercent() / 100.0f);
	fTargetEpRecoveredWhenHit = (float)pVictim->GetCharAtt()->GetLastEpRecoveryWhenHit() + (fDmg * pVictim->GetCharAtt()->GetLastEpRecoveryWhenHitInPercent() / 100.0f);
	...
	//check reflect
	fReflectedDamage = GetReflectDamage(fDmg, byAttackType, pVictim->GetCharAtt()->GetLastPhysicalReflection(), pVictim->GetCharAtt()->GetLastEnergyReflection());
