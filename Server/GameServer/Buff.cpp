...
//		Return	:
//-----------------------------------------------------------------------------------
bool CBuff::IsRevivalAftereffectBuff()
{
	return m_eBuffType == BUFF_TYPE_AFTEREFFECT;
}
		...
		switch (m_aeEffectCode[i])
		{
			...
			case ACTIVE_BURN:
			{
				if (m_pOwnerRef->GetStateManager()->IsCharCondition(CHARCOND_INVINCIBLE) == false)
				{
					...
					if (m_pOwnerRef->IsPC() && m_pOwnerRef->GetCurWorld() && m_pOwnerRef->GetCurWorld()->GetNaviInstanceHandle())
					{
						...
						if (m_abyEffectType[i] == 1)
							dmg = ((float)m_pOwnerRef->GetLastMaxLP() * 10.f / 100.f) * m_sBuffInfo.aBuffParameter[i].buffParameter.fParameter / 100.f;
						else
							dmg = m_sBuffInfo.aBuffParameter[i].buffParameter.fParameter;
					...
					m_pOwnerRef->SendEffectAffected(g_pTableContainer->GetSystemEffectTable()->GetEffectTblidx(m_aeEffectCode[i]), GetSourceType(), GetSourceTblidx(),
						dmg, 0.0f, GetCasterHandle());

					if (m_pOwnerRef->OnBuffDamage(m_hCaster, dmg))
						return true; //return true on death

					//recover lp/ep on hit
					m_pOwnerRef->LpEpRecoverOnHit(m_hCaster, dmg);
				}
			}
			break;
			...
			case ACTIVE_LP_AUTO_RECOVER:
			{
				...
				//	sCOMMONCONFIG_VALUE_DATA* commonConfig = g_pTableContainer->GetCommonConfigTable()->FindCommonConfig(m_sBuffInfo.aBuffParameter[0].buffParameter.commonConfigTblidx);
					...
					int recover = m_pOwnerRef->GetLastMaxLP() - m_pOwnerRef->GetCurLP();
			...
			case ACTIVE_EP_AUTO_RECOVER:
			{
				...
				//	sCOMMONCONFIG_VALUE_DATA* commonConfig = g_pTableContainer->GetCommonConfigTable()->FindCommonConfig(m_sBuffInfo.aBuffParameter[0].buffParameter.commonConfigTblidx);
					...
					WORD recover = m_pOwnerRef->GetLastMaxEP() - m_pOwnerRef->GetCurEP();
			...
			{
				m_dwRemainTime[0] = m_sBuffInfo.aBuffParameter[1].buffParameter.dwRemainTime; //set time for next usage possibility
...
void CBuff::OnEffectInActive(bool bRecalculateStats/* = false*/)
{
	for (int i = 0; i < NTL_MAX_EFFECT_IN_SKILL; i++)
	{
		switch (m_aeEffectCode[i])
		{
			...
			{
				if (m_pOwnerRef->GetAirState() == AIR_STATE_ON) m_pOwnerRef->SendCharStateFalling(NTL_MOVE_HEIGHT_DOWN);
				CPlayer* pPlayer = (CPlayer*)m_pOwnerRef;
			}
			break;
			default: break;
	...
	if (m_sBuffInfo.bySourceType == DBO_OBJECT_SOURCE_SKILL)
	{
		if (sSKILL_TBLDAT* pSkill = (sSKILL_TBLDAT*)g_pTableContainer->GetSkillTable()->FindData(m_sBuffInfo.sourceTblidx))
		{
			...
			if (pSkill->bKeep_Effect)
			{
				...
				if (m_hCaster != m_pOwnerRef->GetID())
				{
					CCharacter* pCaster = g_pObjectManager->GetChar(m_hCaster);
					if (pCaster && pCaster->GetCharStateID() == CHARSTATE_KEEPING_EFFECT
						&& pCaster->GetStateManager()->GetCharState() && pCaster->GetStateManager()->GetCharState()->GetPrevStateID() != INVALID_BYTE) //check if caster is inside state.. Otherwise this might be called while caster is leaving keepingeffect state = infinite loop
					{
