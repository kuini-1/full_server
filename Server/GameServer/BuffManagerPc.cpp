...
bool CBuffManagerPc::RegisterLoadBuff(sBUFF_INFO & rBuffInfo, BYTE byBuffGroup)
{
	...
	if (rBuffInfo.bySourceType == DBO_OBJECT_SOURCE_SKILL)
	{
		...
		if (pSkillTbldat)
		{
			...
			if(aeEffectCode[0] == ACTIVE_REVIVAL_AFTEREFFECT || aeEffectCode[1] == ACTIVE_REVIVAL_AFTEREFFECT || 
				aeEffectCode[0] == ACTIVE_REVIVAL_AFTEREFFECT_PCBANG || aeEffectCode[1] == ACTIVE_REVIVAL_AFTEREFFECT_PCBANG )
				buffType = BUFF_TYPE_AFTEREFFECT;
			else if (Dbo_IsForBlessBuff(pSkillTbldat->bySkill_Active_Type))
				buffType = BUFF_TYPE_BLESS;
			else if (Dbo_IsForCurseBuff(pSkillTbldat->bySkill_Active_Type) || Dbo_IsCurseType(pSkillTbldat->bySkill_Active_Type))
	...
	for (int i = 0; i < NTL_MAX_EFFECT_IN_SKILL; i++)
	{
		...
		else if (aeEffectCode[i] == ACTIVE_LP_AUTO_RECOVER || aeEffectCode[i] == ACTIVE_EP_AUTO_RECOVER || aeEffectCode[i] == ACTIVE_MASCOT_VP_AUTO_RECOVERY || aeEffectCode[i] == ACTIVE_MASCOT_EXP_AUTO_PORTION)
		{
...
bool CBuffManagerPc::RegisterBuff(DWORD& rdwKeepTime, eSYSTEM_EFFECT_CODE * effectCode, sDBO_BUFF_PARAMETER * paBuffParameter, HOBJECT hCaster, eBUFF_TYPE buffType, sSKILL_TBLDAT * pSkillTbldat)
{
	if (CBuffManager::RegisterBuff(rdwKeepTime, effectCode, paBuffParameter, hCaster, buffType, pSkillTbldat))
	{
		if (rdwKeepTime > 0)
		{
			CBuffPc* pBuff = new CBuffPc;
			...
			if (pItemTbldat)
			{
				...
				if (pUseItemTbldat)
				{
					if (pBuff->GetBuffInfo()->bActive) //check if buff is active
					{
						...
						if (pUseItemTbldat->dwUse_Allow_Rule_Bit_Flag > 0 && BIT_FLAG_TEST(pUseItemTbldat->dwUse_Allow_Rule_Bit_Flag, MAKE_BIT_FLAG(byWorldRuleType)) == false) //check if buff has to be disabled
						{
							//printf("[DISABLED] [if 2] tblidx %u || byWorldRuleType: %u || use allow bitflag: %u \n", pUseItemTbldat->tblidx, MAKE_BIT_FLAG(byWorldRuleType), pUseItemTbldat->dwUse_Allow_Rule_Bit_Flag);
							pBuff->ActivateBuff(false);
						}
						else
						{
							//printf("[ENABLED] [else 2] tblidx: %u \n", pUseItemTbldat->tblidx);
							pBuff->ActivateBuff(true);
						}

						if (byWorldRuleType == GAMERULE_CCBATTLEDUNGEON && BIT_FLAG_TEST(pUseItemTbldat->dwUse_Allow_Rule_Bit_Flag, MAKE_BIT_FLAG(GAMERULE_CCBATTLEDUNGEON)) == false)
						{
							//printf("tblidx: %u, use allow bitflag: %u, bitflag: %u, real bitflag: %u, world bitflag: %u \n", pUseItemTbldat->tblidx, pUseItemTbldat->dwUse_Allow_Rule_Bit_Flag, pUseItemTbldat->dwUse_Restriction_Rule_Bit_Flag, MAKE_BIT_FLAG(pUseItemTbldat->dwUse_Restriction_Rule_Bit_Flag), MAKE_BIT_FLAG(byWorldRuleType));
							//printf("[DISABLED] [if 1 ] tblidx %u || byWorldRuleType: %u || use allow bitflag: %u \n", pUseItemTbldat->tblidx, MAKE_BIT_FLAG(byWorldRuleType), pUseItemTbldat->dwUse_Allow_Rule_Bit_Flag);
							pBuff->ActivateBuff(false);
						}
						else
						{
							//printf("[ENABLED] [else 1] tblidx: %u \n", pUseItemTbldat->tblidx);
							if (byWorldRuleType == GAMERULE_CCBATTLEDUNGEON)
								pBuff->ActivateBuff(true);
						}
					}
					else
					{
						if (pUseItemTbldat->dwUse_Allow_Rule_Bit_Flag == 0 && byWorldRuleType != GAMERULE_CCBATTLEDUNGEON)
							pBuff->ActivateBuff(true);
						else if (pUseItemTbldat->dwUse_Allow_Rule_Bit_Flag > 0 && byWorldRuleType == GAMERULE_CCBATTLEDUNGEON)
							pBuff->ActivateBuff(true);
					}
					if (pBuff->GetSystemEffectCode(0) != ACTIVE_LP_AUTO_RECOVER && pBuff->GetSystemEffectCode(0) != ACTIVE_EP_AUTO_RECOVER)
					{
