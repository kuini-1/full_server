...
void CNpc::CopyToObjectInfo(sOBJECT_INFO *pObjectInfo, CHARACTERID playerCharID)
{
	...
	switch (GetObjType())
	{
		case OBJTYPE_NPC:
		{
			...
			pObjectInfo->npcBrief.fLastFlyingAccelSpeed = GetLastFlyAccelSpeed();
			pObjectInfo->npcBrief.fLastFlyingDashSpeed = GetLastFlyDashSpeed();
			pObjectInfo->npcBrief.fLastFlyingSpeed = GetLastFlySpeed();
			pObjectInfo->npcBrief.fLastRunningSpeed = GetLastRunSpeed();
			pObjectInfo->npcBrief.fLastWalkingSpeed = GetWalkingSpeed();
			pObjectInfo->npcBrief.fSkillAnimationSpeedModifier = GetLastSkillAnimationSpeedModifier();
			pObjectInfo->npcBrief.maxLp = GetLastMaxLP();
			pObjectInfo->npcBrief.nicknameTblidx = bot_profile.nicknameTblidx;
			pObjectInfo->npcBrief.tblidx = GetTblidx();
			pObjectInfo->npcBrief.wAttackSpeedRate = GetLastAttackSpeedRate();
			pObjectInfo->npcBrief.wCurEP = GetCurEP();
			pObjectInfo->npcBrief.wMaxEP = GetLastMaxEP();
		}
		break;
...
bool CNpc::Recover(DWORD dwTickDiff)
{
	...
	AcquireLpEpEventID();
	UpdateCurLpEp((int)GetCharAtt()->GetLastLPSitdownRegen(), GetCharAtt()->GetLastEPSitdownRegen(), true, true);
...
float CNpc::GetAttackRange(CCharacter* pTarget)
{
	if (pTarget)
	{
		return GetAttackRange() + GetObjectRadius() + pTarget->GetObjectRadius();
	}
...
bool CNpc::HasNearbyPlayerInQuadArea()
{
	...
	if (GetTickCount() - m_dwLastHasNearbyPlayerQuadTime >= 1000)
	{
		m_bCacheHasNearbyPlayerQuad = false;
		m_dwLastHasNearbyPlayerQuadTime = GetTickCount();
...
{
	CGameServer* app = (CGameServer*)g_pApp;
	if (app && app->IsMonsterAggressive() && IsMonster())
	{
		if (dwAIBitFlag == AI_FUNC_FLAG_OFFENSIVE || dwAIBitFlag == AI_FUNC_FLAG_SMART_OFFENSIVE)
			return true;
	}
...
bool CNpc::AttackProgress(DWORD dwTickDiff, float fMultiple)
{
	...
	//	printf("fAnimationTime %f, fAttackAnimationSpeed %f, fRequiredAttackAnimationTime %f, GetLastAttackSpeedRate() %u\n", fAnimationTime, fAttackAnimationSpeed, fRequiredAttackAnimationTime, GetLastAttackSpeedRate());
		return true;
	}
...
bool CNpc::ConsiderAttackRange(float fBreakupPos)
{
	...
	if (pTarget)
	{
		...
		{
			if (IsInRange3(pTarget, GetAttackRange() * (fBreakupPos + 1.0f) ))
				return true;
		}
		else
		{
			if (IsInRange(pTarget, GetAttackRange() * (fBreakupPos + 1.0f)))
				return true;
		}
	}
