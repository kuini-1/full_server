...
int CStateManager::OnUpdate(DWORD dwTickDiff, float fMultiple)
{
	if (GetCharState())
	{
		GetCharState()->SetStateTime(GetCharState()->GetStateTime() + dwTickDiff);

		GetCharState()->OnUpdate(dwTickDiff, fMultiple);

		if (GetCharState()->IsFinished())
		{
			if (GetCharState()->GetNextStateID() == INVALID_BYTE)
			{
				ERR_LOG(LOG_GENERAL, "GetCharState()->GetNextStateID() == INVALID_BYTE");
				return 1;
			}

			if (ChangeCharState(GetCharState()->GetNextStateID(), NULL, true))
				return 2;
		}
	}
...
bool CStateManager::ChangeCharState(BYTE stateID, void *pvInitialData, bool bUpdate)
{
	...
	if (m_pCharState)
	{
		...
		//if(m_pCharState->GetStateID() == CHARSTATE_PARALYZED)
		//	NTL_PRINT(PRINT_APP, "Change state %u -> %u", m_pCharState->GetStateID(), stateID);

		//this 2 MUST be before OnExist
		m_pCharState->SetPrevStateID(INVALID_BYTE);
		m_pCharState->SetNextStateID(INVALID_BYTE);
...
int CStateManager::CopyTo(sCHARSTATE* pCharState)
{
	...
	if (GetCharState())
	{
		...
		pCharState->sCharStateBase.bIsValidLoc = true;
