...
public:
	...
	void		ReEnterBattleDungeon(CPlayer* pPartyLeader, CPlayer* pMember, CBattleDungeon* pDungeon, CWorld* pWorld, CScriptAlgoObject* pScript);//ccreconnect
	...
	template <class Func> void ForEachMemberCopy(Func& f); //ccreconnect
...
private:
	...
	TBLIDX						m_registeredCharmTblidx[NTL_PARTY_CHARM_INVENTORY_COUNT];
	DWORD						m_adwRemainingTime[NTL_PARTY_CHARM_INVENTORY_COUNT];
	...
	sPARTY_MEMBER_INFO			m_memberInfoCopy[NTL_MAX_MEMBER_IN_PARTY]; //ccreconnect
...
template <class Func> void CParty::ForEachMemberCopy(Func& f) //ccreconnect
{
	for (int i = 0; i < m_byMemberInfoCount; i++)
	{
		m_memberInfoCopy[i] = m_memberInfo[i];
		CPlayer* player = g_pObjectManager->GetPC(m_memberInfo[i].hHandle);
		if (player)
			f(player);
	}
