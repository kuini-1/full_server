...
void CQueryServerSession::RecvItemMoveStackRes(CNtlPacket * pPacket)
{
	...
	if (req->wResultCode == GAME_SUCCESS)
	{
		if (hDestItem == INVALID_HOBJECT) //check if new item has been created (UNSTACK)
		{
			...
			res->bIsNew = true;
			res->handle = pDestItem->GetID();
			memcpy(&res->sItemData, &pDestItem->GetItemData(), sizeof(sITEM_DATA));
...
void CQueryServerSession::RecvMailStartRes(CNtlPacket * pPacket)
{
	...
	res2->byPartsType = req->byPartsType;
...
void CQueryServerSession::RecvSkillInitRes(CNtlPacket* pPacket)
{
	...
	if (req->wResultCode == GAME_SUCCESS)
	{
		...
		pOwner->UpdateCharSP(((DWORD)pOwner->GetLevel() - 1) + pOwner->GetSkillPointsBought());
...
void CQueryServerSession::RecvGiftShopItemBuyRes(CNtlPacket* pPacket)
{
	sQG_GIFT_SHOP_BUY_RES* req = (sQG_GIFT_SHOP_BUY_RES*)pPacket->GetPacketData();
...
void CQueryServerSession::RecvPcDataLoadRes(CNtlPacket * pPacket)
{
	...
	if (pOwner == NULL)
	{
		ERR_LOG(LOG_USER,"player %u not found \n", req->sPcData.charId);
		return;
	}
	...
	pOwner->RecvLoadPcDataRes(&req->sPcData, &req->serverChangeInfo, req->bTutorialFlag, &req->sWarFogInfo, &req->sMailBrief, &req->sRankBattleScore, req->TitleIndexFlag, req->wWaguCoins, req->wEventCoins, &req->sPcNewData, &req->sAccNewData);
}
...
void CQueryServerSession::RecvCashItemMoveRes(CNtlPacket * pPacket)
{
	...
	if (req->wResultCode == GAME_SUCCESS)
	{
		...
		sGU_CASHITEM_DEL_NFY * res2 = (sGU_CASHITEM_DEL_NFY *)packet2.GetPacketData();
		res2->wOpCode = GU_CASHITEM_DEL_NFY;
		res2->qwProductId = req->qwProductId;
...
void CQueryServerSession::RecvCashitemSendGiftRes(CNtlPacket* pPacket)
{
	...
	*/
	...
	//packet for sender
	if (req->DestAccountId == INVALID_ACCOUNTID)
	{
	...
	if (req->wResultCode == GAME_SUCCESS && req->DestAccountId != INVALID_ACCOUNTID)
	{
		...
		res->wOpCode = GU_CASHITEM_ADD_NFY;
		res->sInfo.qwProductId = req->qwProductId;
		res->sInfo.HLSitemTblidx = req->dwIdxHlsTable;
		res->sInfo.byStackCount = req->byCount;
		res->sInfo.giftCharId = pReceiver->GetCharID();
		res->sInfo.nIsRead = 0;
		res->sInfo.tRegTime.day = req->tRegTime.day;
		res->sInfo.tRegTime.hour = req->tRegTime.hour;
		res->sInfo.tRegTime.minute = req->tRegTime.minute;
		...
		pReceiver->AddCashShopItem(res->sInfo.qwProductId, &res->sInfo);
	}
}
...
void CQueryServerSession::RecvCharacterRenameRes(CNtlPacket * pPacket)
{
	...
	res->wResultCode = req->wResultCode;
	res->dwRemainMin = req->dwRemainMin;
