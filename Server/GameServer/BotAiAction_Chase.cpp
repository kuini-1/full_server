...
int CBotAiAction_Chase::OnUpdate(DWORD dwTickDiff, float fMultiple)
{
	...
	else
	{
		if (bIsChasingCheatingTarget)
		{
			if (GetBot()->IsInRange(GetBot()->GetCheatingTargetCollisionLoc(), GetBot()->GetAttackRange() * 70.0f / 100.0f))
			{
				m_status = COMPLETED;
				return m_status;
			}
		...
		struct sFOREACH
		{
			...
			void operator () (std::pair<HOBJECT, HOBJECT> value)
			{
				...
				if (pAssociate)
				{
					...
					{
						if (pAssociate->GetAttackRange() > 2.0f)
						{
							if (pTarget->IsInRange(pAssociate, pAssociate->GetAttackRange()))
								++byCount;
						}
						else
						{
