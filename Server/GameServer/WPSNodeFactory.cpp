...
CControlScriptNodeAction* CWPSNodeFactory::CreateControlNodeAction(const char* lpszNodeName)
{
	...
	std::unordered_map<std::string, CControlScriptNodeAction*(*)(const char*)>::iterator it = s_actionMap_2.find(lpszNodeName);
...
CControlScriptNodeCondition* CWPSNodeFactory::CreateControlNodeCondition(const char* lpszNodeName)
{
	...
	std::unordered_map<std::string, CControlScriptNodeCondition*(*)(const char*)>::iterator it = s_conditionMap_2.find(lpszNodeName);
