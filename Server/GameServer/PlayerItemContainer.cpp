...
void CPlayerItemContainer::SetItemBrief(BYTE byPos, sITEM_DATA * pItemData)
{
	...
	m_sItemBrief[byPos].byRank = pItemData->byRank;
...
{
	// Skip equipment stats in battle royale world
	if (m_pOwner->GetWorldTblidx() == 212000)
		return;
	...
	for (BYTE a = 0; a <= EQUIP_SLOT_TYPE_BOOTS; a++)
	{
		...
		if (item && item->GetDurability() > 0)
		{
			...
			if (pItemTbldat)
			{
				if (m_pOwner->GetWorldTblidx() == 910000 && pItemTbldat->wScouter_Watt != 555) break;
				...
				if (item->GetPos() == EQUIP_SLOT_TYPE_HAND)
				{
					if (pItemTbldat->wPhysical_Offence != INVALID_WORD)
						avt.wBasePhysicalOffence += Dbo_GetFinalOffence(pItemTbldat->wPhysical_Offence, item->GetGrade());

					if (pItemTbldat->wEnergy_Offence != INVALID_WORD)
						avt.wBaseEnergyOffence += Dbo_GetFinalOffence(pItemTbldat->wEnergy_Offence, item->GetGrade());
				}

				if (pItemTbldat->wAttack_Speed_Rate != INVALID_WORD)
					avt.wBaseAttackSpeedRate = pItemTbldat->wAttack_Speed_Rate;

				if (pItemTbldat->wPhysical_Defence != INVALID_WORD)
					avt.wBasePhysicalDefence += Dbo_GetFinalDefence(pItemTbldat->wPhysical_Defence, item->GetGrade());

				if (pItemTbldat->wEnergy_Defence != INVALID_WORD)
					avt.wBaseEnergyDefence += Dbo_GetFinalDefence(pItemTbldat->wEnergy_Defence, item->GetGrade());

				if (pItemTbldat->fAttack_Range_Bonus != INVALID_FLOAT)
					avt.fBaseAttackRange += pItemTbldat->fAttack_Range_Bonus;
			}
		}
	}
}
...
void CPlayerItemContainer::CopyItemAttributesTo(CCharacterAtt* pCharAtt)
{
	...
	for (BYTE a = 0; a < EQUIP_SLOT_TYPE_COUNT; a++)
	{
		...
		if (item /*item->GetPlace() == CONTAINER_TYPE_EQUIP*/ && item->GetDurability() > 0)
		{
			...
			if (pinvitemdata)
			{
				if (m_pOwner->GetWorldTblidx() == 910000 && pinvitemdata->wScouter_Watt != 555) break;
				if (item->GetPos() == EQUIP_SLOT_TYPE_HAND)
				{
				...
				{
					m_pOwner->GetCharAtt()->SetBattleAttributeOffenceSub(item->GetBattleAttribute());
				...
				if (item->GetPos() >= EQUIP_SLOT_TYPE_HAND)//EQUIP_SLOT_TYPE_COSTUME_SET) // dogi ball
				{
