		...
		if (m_pTargetListManager)
		{
			if (m_cBotBreakUp.Create(this))
			{
				if (m_cMeHaterManager.Create(this))
				{
					...
					if (sCharState->sCharStateBase.eAirState == AIR_STATE_ON)
						SetMoveStatus(NTL_MOVE_STATUS_AIR_MOVE);
					else
...
float CCharacter::GetCurSpeed()
{
	...
	switch (GetMoveFlag())
	{
		case NTL_MOVE_FLAG_RUN: fSpeed = GetLastRunSpeed(); break;
		case NTL_MOVE_FLAG_FLY: fSpeed = GetLastFlySpeed(); break;
		case NTL_MOVE_FLAG_FLY_DASH: fSpeed = GetLastFlyDashSpeed(); break;
		case NTL_MOVE_FLAG_FLY_ACCEL: fSpeed = GetLastFlyAccelSpeed(); break;
		case NTL_MOVE_FLAG_FLY_QUICK_DOWN: fSpeed = GetLastFlyAccelSpeed() * 1.35f; break;
...
bool CCharacter::UpdateFollow(DWORD dwTickDiff, float fMultiple)
{
	...
	CTriggerObject* pObject = GetCurWorld()->FindStaticObject(GetTargetHandle());
	if (pObject)
	{
		return CMoveableObject::UpdateFollow(dwTickDiff, fMultiple);
	}
...
void CCharacter::EnterConfusedState(HOBJECT hCaster)
{
	...
	if (pTarget)
	{
		...
		if (SendCharStateFollowing(hTarget, GetAttackRange(), DBO_MOVE_FOLLOW_AUTO_ATTACK, vDestLoc, true) == false)
			return;
...
void CCharacter::EnterTerrorState(HOBJECT hCaster)
{
	...
	if (pCaster)
	{
		...
		if (SendCharStateMoving(NTL_MOVE_F, NTL_MOVE_FLAG_RUN, NTL_MOVE_STATUS_NONE) == false)
			return;
...
void CCharacter::InsertKeepEffectTarget(HOBJECT hObj, TBLIDX skillIdx)
{
	m_mapKeepEffectTargets.insert(std::make_pair(hObj, skillIdx));
}

void CCharacter::EraseKeepEffectTarget(HOBJECT hObj)
{
	m_mapKeepEffectTargets.erase(hObj);
}

void CCharacter::RemoveKeepEffectTargets()
{
	for (std::map<HOBJECT, TBLIDX>::iterator it = m_mapKeepEffectTargets.begin(); it != m_mapKeepEffectTargets.end(); )
	{
		HOBJECT hId = it->first;
		TBLIDX idx = it->second;
		...
		//	if(pChar->GetID() == GetID())
				pChar->GetBuffManager()->EndBuffTS(idx, CBuff::BUFF_REMOVAL_REASON_BY_ITSELF);//release myself in next tick to avoid endless loop
		//	else
		//		pChar->GetBuffManager()->EndBuff(idx, CBuff::BUFF_REMOVAL_REASON_BY_ITSELF); // if we do this, then we crash client if 2 chars use skill like "Electrical Discharge" and then 1 leaves.
		}
	}
...
{
	return GetAttackRange() + GetObjectRadius();
}
...
void CCharacter::LpEpRecoverOnHit(HOBJECT hCaster, float fDmg)
{
	...
	//check lp
	res->lpEpRecovered.targetLpRecoveredWhenHit = (int)(GetCharAtt()->GetLastLpRecoveryWhenHit() + (fDmg * GetCharAtt()->GetLastLpRecoveryWhenHitInPercent() / 100.0f));
	if (res->lpEpRecovered.targetLpRecoveredWhenHit > 0)
		res->lpEpRecovered.bIsLpRecoveredWhenHit = true;
	//check ep
	res->lpEpRecovered.dwTargetEpRecoveredWhenHit = (DWORD)(GetCharAtt()->GetLastEpRecoveryWhenHit() + (fDmg * GetCharAtt()->GetLastEpRecoveryWhenHitInPercent() / 100.0f));
...
bool CCharacter::CannotMoveByBuff()
{
	if (GetCharStateID() == CHARSTATE_STUNNED)
		...
		return true;
	else if (GetCharStateID() == CHARSTATE_SLIDING)
		return true;
...
//		SET STANDING STATE
//--------------------------------------------------------------------------------------//
void	CCharacter::SendCharStateStanding()
{
	if (GetCharStateID() == CHARSTATE_STANDING)
		return;
...
bool CCharacter::SendCharStateSkillAffecting(TBLIDX skillid, HOBJECT hTarget)
{
	...
	res->sCharState.sCharStateBase.bIsValidLoc = true;
	res->sCharState.sCharStateBase.dwConditionFlag = GetConditionState();
	res->sCharState.sCharStateBase.dwStateTime = GetTickCount();
...
//--------------------------------------------------------------------------------------//
void	CCharacter::SendCharStateKeepingEffect()
{
	if (GetCharStateID() == CHARSTATE_KEEPING_EFFECT) // by kuini test
	{
		RemoveKeepEffectTargets(); //fix against people using Playfull Doll (Karma AOE Confusion) and then Play Dead to keep the confusion active
		return;
	}
...
void	CCharacter::SendCharStateFalling(BYTE byMoveDirection)
{
	...
	res->sCharState.sCharStateBase.dwConditionFlag = GetConditionState();
	res->sCharState.sCharStateBase.dwStateTime = 0;
	res->sCharState.sCharStateDetail.sCharStateFalling.byMoveFlag = NTL_MOVE_FLAG_FLY_QUICK_DOWN;
...
//		SET FOLLOWING STATE
//--------------------------------------------------------------------------------------//
bool CCharacter::SendCharStateFollowing(HOBJECT hTarget, float fDistance, BYTE byMoveReason, sVECTOR3& vDestLoc, bool bRunMode)
{
	CNtlPacket packet(sizeof(sGU_UPDATE_CHAR_STATE));
	sGU_UPDATE_CHAR_STATE * res = (sGU_UPDATE_CHAR_STATE *)packet.GetPacketData();
	res->wOpCode = GU_UPDATE_CHAR_STATE;
	...
	{
		res->sCharState.sCharStateDetail.sCharStateDestMove.byMoveFlag = GetMoveFlag();
	}
	else
	{
		if (bRunMode)
	...
	res->sCharState.sCharStateDetail.sCharStateFollwing.byMoveStatus = NTL_MOVE_STATUS_NONE;
...
//--------------------------------------------------------------------------------------//
void	CCharacter::SendCharStateAirDashAccel(BYTE byMoveDir)
{
	CNtlPacket packet(sizeof(sGU_UPDATE_CHAR_STATE));
	sGU_UPDATE_CHAR_STATE* res = (sGU_UPDATE_CHAR_STATE*)packet.GetPacketData();
	res->handle = this->GetID();
...
//		SET MOVING STATE
//--------------------------------------------------------------------------------------//
bool	CCharacter::SendCharStateMoving(BYTE byMoveDir, BYTE byFlag, BYTE byStatus)
{
	CNtlPacket packet(sizeof(sGU_UPDATE_CHAR_STATE));
	sGU_UPDATE_CHAR_STATE * res = (sGU_UPDATE_CHAR_STATE *)packet.GetPacketData();
	res->wOpCode = GU_UPDATE_CHAR_STATE;
	...
	res->sCharState.sCharStateDetail.sCharStateMoving.byMoveStatus = byStatus;
	res->sCharState.sCharStateDetail.sCharStateMoving.dwTimeStamp = 0;
	packet.SetPacketLen(sizeof(sGU_UPDATE_CHAR_STATE));
...
void CCharacter::StandUpIfPossible(BYTE byEndState, WORD wEffectCodeEnd/* = INVALID_WORD*/)
{
	...
	*/
	...
	if (GetCharStateID() == CHARSTATE_FAINTING || GetCharStateID() == CHARSTATE_SLIDING || GetCharStateID() == CHARSTATE_KNOCKDOWN 
		|| GetCharStateID() == CHARSTATE_SANDBAG || GetCharStateID() == CHARSTATE_GUARD || GetCharStateID() == CHARSTATE_PRIVATESHOP)
		return;
