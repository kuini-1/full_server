...
#include "NtlRandom.h"
#include "DungeonManager.h"//ccreconnect
#include "CPlayer.h"//ccreconnect
...
int CWpsAlgoAction_AddMobGroup::OnUpdate(DWORD dwTickDiff, float fMultiple)
{
	...
	//NTL_PRINT(PRINT_APP, "spawnCount: %u, CCBD-STAGE %u, m_spawnGroupId %u, m_mapBindMob.size %zu, m_nBindMobList", 
	...
	else
	{
		...
		//CBattleDungeon* pDungeon = g_pDungeonManager->FindBattleDungeon(m_partyId);//ccreconnect
		//pDungeon->SetCCBDCurStage(GetOwner()->GetCCBDStage());//ccreconnect
