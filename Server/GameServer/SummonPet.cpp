...
void CSummonPet::CopyToObjectInfo(sOBJECT_INFO * pObjectInfo, CHARACTERID playerCharID)
{
	...
	pObjectInfo->summonPetBrief.fLastRunningSpeed = GetLastRunSpeed();
	pObjectInfo->summonPetBrief.fLastAttackRange = GetAttackRange();
	pObjectInfo->summonPetBrief.fSkillAnimationSpeedModifier = GetLastSkillAnimationSpeedModifier();
	pObjectInfo->summonPetBrief.curLp = GetCurLP();
	pObjectInfo->summonPetBrief.wCurEP = GetCurEP();
	pObjectInfo->summonPetBrief.maxLp = GetLastMaxLP();
	pObjectInfo->summonPetBrief.wMaxEP = GetLastMaxEP();
	pObjectInfo->summonPetBrief.wAttackSpeedRate = GetLastAttackSpeedRate();
...
void CSummonPet::Spawn(CPlayer* pOwner)
{
	...
	resPet->sCharState.sCharStateBase.bIsValidLoc = true;
