...
bool CPlayer::EquipItem(CItem* item, BYTE byDestpos)
{
	...
	res->sItemBrief.byRank = pItemData->byRank;
...
bool CPlayer::UnequipItem(CItem* item)
{
	...
	res->sItemBrief.tblidx = INVALID_TBLIDX;
	packet.SetPacketLen(sizeof(sGU_UPDATE_ITEM_EQUIP));
	Broadcast(&packet);
...
bool CPlayer::StartVehicle(TBLIDX vehicleIdx, HOBJECT hVehicleId)
{
	...
	if (pVehicleTbl)
	{
		...
		*/
		...
		pRes2->aspectState.sAspectStateDetail.sVehicle.bIsEngineOn = false;
...
void CPlayer::EndVehicle(WORD wReasonCode)
{
	...
	res4->sCharState.sCharStateBase.bIsValidLoc = true;
	...
	res4->sCharState.sCharStateBase.aspectState.sAspectStateDetail.sVehicle.bIsEngineOn = false;
	...
	else
	{
		if (GetVehicleFuelId() != INVALID_HOBJECT)
		{
			...
			if (pFuel)
			{
				...
				CNtlPacket packet(sizeof(sGU_VEHICLE_FUEL_UPDATE_NFY));
				sGU_VEHICLE_FUEL_UPDATE_NFY * res = (sGU_VEHICLE_FUEL_UPDATE_NFY *)packet.GetPacketData();
				res->wOpCode = GU_VEHICLE_FUEL_UPDATE_NFY;
				res->wResultCode = GAME_SUCCESS;
				packet.SetPacketLen(sizeof(sGU_VEHICLE_FUEL_UPDATE_NFY));
				SendPacket(&packet);
...
void CPlayer::SetVehicle(TBLIDX vehicleTblidx, HOBJECT hVehicleId, bool bGetOffOnAttack)
{
	...
	m_vehicle.bIsVehicleEngineOn = false;
}
