...
bool CBuffManager::Create(CCharacter *pOwnerRef)
{
	...
	if (m_blessBuffList.Create(NTL_MAX_BLESS_BUFF_CHARACTER_HAS, 0))
	{
		if (m_curseBuffList.Create(NTL_MAX_CURSE_BUFF_CHARACTER_HAS, 0))
		{
			...
			{
				if (m_revivalAftereffectBuffList.Create(NTL_MAX_REVIVAL_AFTEREFFECT_BUFF_CHARACTER_HAS, 0))
					return true;
			}
		}
	}
...
void CBuffManager::Destroy()
{
	...
	//CLEAR REVIVAL AFTER EFFECT BUFF
	for (unsigned int pos = m_revivalAftereffectBuffList.Begin();
		m_revivalAftereffectBuffList.End() != pos;
		pos = m_revivalAftereffectBuffList.Next(pos))
	{
		CBuff* pBuff = m_revivalAftereffectBuffList.GetAt(pos);
		if (pBuff)
		{
			SAFE_DELETE(pBuff);
			m_revivalAftereffectBuffList.GetAt(pos) = NULL;
		}
	}
	m_revivalAftereffectBuffList.Clear();
...
bool CBuffManager::RegisterBuff(DWORD& rdwKeepTime, eSYSTEM_EFFECT_CODE * effectCode, sDBO_BUFF_PARAMETER * paBuffParameter, HOBJECT hCaster, eBUFF_TYPE buffType, sSKILL_TBLDAT * pSkillTbldat)
{
	...
	{
		if (m_subBuffList.GetSize() >= NTL_MAX_BLESS_BUFF_CHARACTER_HAS)
			return false;
	}
	break;
	case BUFF_TYPE_CURSE:
	{
		if (m_subBuffList.GetSize() >= NTL_MAX_CURSE_BUFF_CHARACTER_HAS)
			return false;
	}
	break;
	case BUFF_TYPE_AFTEREFFECT:
	{
		if (m_subBuffList.GetSize() >= NTL_MAX_REVIVAL_AFTEREFFECT_BUFF_CHARACTER_HAS)
			return false;
	}
	break;
	...
	{
		rdwKeepTime -= DWORD((float)rdwKeepTime * m_pOwnerRef->GetCrowdControlReduction(hCaster, g_pTableContainer->GetSkillTable()->FindBasicSkillTblidx(pSkillTbldat->tblidx)) / 100.0f);
	}
	...
	if (pOldBuff)
	{
		...
		if (pSkillTbldat->tblidx == pOldBuff->GetSourceTblidx() || (bOverwrite && pOldBuff->GetBuffGroup() == pSkillTbldat->byBuff_Group))
		{
...
void CBuffManager::EnableDisableBuff(BYTE byBuffIndex, eBUFF_TYPE buffType)
{
	...
	switch (buffType)
	{
		...
		case BUFF_TYPE_AFTEREFFECT:
		{
			for (unsigned int pos = m_revivalAftereffectBuffList.Begin(); pos != m_revivalAftereffectBuffList.End(); pos = m_revivalAftereffectBuffList.Next(pos))
			{
				CBuff* pBuff = m_revivalAftereffectBuffList.GetAt(pos);
				if (pBuff && pBuff->GetBuffIndex() == byBuffIndex)
				{
	...
	switch (buffType)
	{
		...
		case BUFF_TYPE_AFTEREFFECT:
		{
			for (unsigned int pos = m_revivalAftereffectBuffList.Begin(); pos != m_revivalAftereffectBuffList.End(); pos = m_revivalAftereffectBuffList.Next(pos))
			{
				...
				if (pBuff && pBuff->GetBuffIndex() == byBuffIndex)
				{
					RemoveBuff_Internal(nReason, &m_revivalAftereffectBuffList, pos);
					return;
				}
			}
		}
...
{
	if (pBuff->IsRevivalAftereffectBuff())
		return AddBuff_Internal(pBuff, m_revivalAftereffectBuffList, NTL_MAX_REVIVAL_AFTEREFFECT_BUFF_CHARACTER_HAS, true, false);
	else if (pBuff->IsSubBuff())
...
{
	if (pBuff->IsRevivalAftereffectBuff())
		return AddBuff_Internal(pBuff, m_revivalAftereffectBuffList, NTL_MAX_REVIVAL_AFTEREFFECT_BUFF_CHARACTER_HAS, false, bNeedToDisplayMessage);
	else if (pBuff->IsSubBuff())
...
//		Return	:
//-----------------------------------------------------------------------------------
void CBuffManager::RemoveAllMainBuff()
{
...
//		Return	:
//-----------------------------------------------------------------------------------
void CBuffManager::RemoveAllRevivalAfterEffectBuff()
{
	unsigned int pos = m_revivalAftereffectBuffList.Begin();
	while (pos != m_revivalAftereffectBuffList.End())
	{
		CBuff* pBuff = m_revivalAftereffectBuffList.GetAt(pos);
		if (pBuff)
			pos = RemoveBuff_Internal(CBuff::BUFF_REMOVAL_REASON_ETC, &m_revivalAftereffectBuffList, pos);
		else
			pos = m_revivalAftereffectBuffList.Next(pos);
	}
}
...
bool CBuffManager::RemoveRandomCurseBuff(bool bDontRemoveBodyCurse/* = false*/)
{
	...
	unsigned int rand = vecTemp[RandomRangeU(0, vecTemp.size() - 1)];
					...
					if (byEffectType[j] != INVALID_BYTE)
					{
						...
						if (effectcode != INVALID_SYSTEM_EFFECT_CODE)
						{
							if (effectcode == ACTIVE_RABIES)
							{
								...
								if (commonConfig)
								{
									pCharAtt->CalculateLastAttackSpeedRate((float)commonConfig->adwValue[0], SYSTEM_EFFECT_APPLY_TYPE_PERCENT, false);
									rAvatarAttribute.wLastPhysicalOffence += WORD((float)rAvatarAttribute.wLastPhysicalOffence * ((float)commonConfig->adwValue[1]) / 100.f);
							...
							else if (effectcode == ACTIVE_DRUNK)
							{
								...
								if (commonConfig)
								{
									pCharAtt->CalculateLastAttackSpeedRate((float)commonConfig->adwValue[0], SYSTEM_EFFECT_APPLY_TYPE_PERCENT, true);
									rAvatarAttribute.fLastSkillAnimationSpeedModifier -= rAvatarAttribute.fLastSkillAnimationSpeedModifier * ((float)commonConfig->adwValue[1]) / 100.f;
							...
							else if (effectcode == ACTIVE_EXCITATION_FEMALE)
							{
								...
								if (commonConfig)
								{
									rAvatarAttribute.wLastPhysicalDefence += WORD((float)rAvatarAttribute.wLastPhysicalDefence * ((float)commonConfig->adwValue[0]) / 100.f);
									rAvatarAttribute.wLastEnergyDefence += WORD((float)rAvatarAttribute.wLastEnergyDefence * ((float)commonConfig->adwValue[0]) / 100.f);
									break;
								}
							}
							else
...
void CBuffManager::SendBuffInfo(CCharacter* pPlayer)
{
		...
		pBuffLink = (CBuffLinkedObject*)pBuffLink->GetNext())
	{
		...
		if (pBuff)
		{
			...
			res->aBuffInfo[res->byCount++] = *pBuffInfo;
		}
	}

	if (res->byCount > 0)
	{
		packet.SetPacketLen(sizeof(sGU_BUFF_REFRESH_ALL));
