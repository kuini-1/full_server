...
{
	UNREFERENCED_PARAMETER(byServerType);
...
{
	std::unordered_map<ACCOUNTID, _SERVER_PAIR_ID>::iterator it = m_map_GameServerPlayers.find(AccID);
...
{
	for (std::unordered_map<ACCOUNTID, SERVERINDEX>::iterator it = m_map_CharServerPlayers.begin(); it != m_map_CharServerPlayers.end(); )
	{
...
{
	for (std::unordered_map<ACCOUNTID, _SERVER_PAIR_ID>::iterator it = m_map_GameServerPlayers.begin(); it != m_map_GameServerPlayers.end(); )
	{
...
{
	std::unordered_map<ACCOUNTID, SERVERINDEX>::iterator it = m_map_CharServerPlayers.find(AccID);
	...
	std::unordered_map<ACCOUNTID, _SERVER_PAIR_ID>::iterator it2 = m_map_GameServerPlayers.find(AccID);
