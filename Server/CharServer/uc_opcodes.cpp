...
void CUC_Opcodes::BuildOpcodeList()
{
	...
	StoreOpcode(UC_CASHITEM_HLSHOP_REFRESH_REQ, "UC_CASHITEM_HLSHOP_REFRESH_REQ", PROCESS_THREADUNSAFE, &CClientSession::OnInvalid);
	StoreOpcode(UC_CASHITEM_BUY_REQ, "UC_CASHITEM_BUY_REQ", PROCESS_THREADUNSAFE, &CClientSession::OnInvalid);
	StoreOpcode(UC_CHAR_SERVERLIST_REQ, "UC_CHAR_SERVERLIST_REQ", PROCESS_THREADUNSAFE, &CClientSession::SendServerListReq);
}
