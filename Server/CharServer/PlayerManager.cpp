...
void CPlayerManager::DisconnectAll()
{
	...
	for (std::unordered_map<ACCOUNTID, CPlayer*>::iterator it = m_map_Player.begin(); it != m_map_Player.end(); it++)
	{
...
void CPlayerManager::RemovePlayer(CPlayer* pPlayer)
{
	...
	std::unordered_map<ACCOUNTID, CPlayer*>::iterator it2 = m_map_Player.find(pPlayer->GetAccountID());
...
{
	m_mutex.lock();
	std::unordered_map<ACCOUNTID, CPlayer*>::iterator it = m_map_Player.find(AccountID);
...
CPlayer* CPlayerManager::CreatePlayer(ACCOUNTID AccountID, CClientSession* pSession)
{
	...
	std::unordered_map<ACCOUNTID, CPlayer*>::iterator it = m_map_Player.find(AccountID); //check if already online
	if (it != m_map_Player.end())
	{
		m_mutex.unlock();
		return NULL;
