...
int CMasterServerSession::OnConnect()
{
	...
	NTL_STRCPY_S(res->sServerInfo.achPublicAddress, NTL_MAX_LENGTH_OF_IP + 1, app->m_config.strPublicClientAcceptAddr.c_str());
	printf("[CharServer] Registering with Master Server: PublicAddress='%s', Port=%u, Listening on '%s'\n", app->m_config.strPublicClientAcceptAddr.c_str(), app->m_config.wClientAcceptPort, app->m_config.strClientAcceptAddr.c_str());
