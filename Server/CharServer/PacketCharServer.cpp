...
// Load farm info
	this->LoadServerFarmInfo();

// Load Server list
	this->LoadOneServerList(NULL);

	CNtlPacket packet(sizeof(sCU_CHAR_SERVERLIST_RES));
	sCU_CHAR_SERVERLIST_RES * res = (sCU_CHAR_SERVERLIST_RES *)packet.GetPacketData();
	res->wOpCode = CU_CHAR_SERVERLIST_RES;
	res->wResultCode = CHARACTER_SUCCESS;
	packet.SetPacketLen(sizeof(sCU_CHAR_SERVERLIST_RES));
...
void CClientSession::SendCharCreateReq(CNtlPacket * pPacket)
{
	...
	else if (req->byRace == RACE_HUMAN)
	{
		if (req->byClass != PC_CLASS_HUMAN_FIGHTER && req->byClass != PC_CLASS_HUMAN_MYSTIC) //check if creating valid human class
			resultcode = CHARACTER_FAIL;
		else if (req->byHairColor > DBO_CHAR_HAIR_COLOR_COUNT)
			resultcode = CHARACTER_FAIL;
	}
	else if (req->byRace == RACE_NAMEK)
	{
		if(req->byClass != PC_CLASS_NAMEK_FIGHTER && req->byClass != PC_CLASS_NAMEK_MYSTIC) //check if creating valid namekian class
			resultcode = CHARACTER_FAIL;
		else if (req->byHairColor > DBO_CHAR_SKIN_COLOR_COUNT)
			resultcode = CHARACTER_FAIL;
	}
	...
	if (resultcode == CHARACTER_SUCCESS)
	{
		...
		//	resultcode = CHARACTER_BLOCK_STRING_INCLUDED;
			...
			resultcode = CHARACTER_FAIL;
		else if (app->m_config.bReserveName)
		{
			if (g_pReserveName->IsReserved(charname, GetPlayer()->GetAccountID()))
	...
	if (resultcode == CHARACTER_SUCCESS)
	{
		...
		{
			res->asQuickSlotData[i].tbilidx = NewbieTblData->asQuickData[i].tbilidx;
			res->asQuickSlotData[i].byQuickSlot = NewbieTblData->asQuickData[i].byQuickSlot;
			res->asQuickSlotData[i].byType = NewbieTblData->asQuickData[i].byType;
		}
...
void CClientSession::SendCharDeleteReq(CNtlPacket * pPacket)
{
	...
	char* password = Ntl_WC2MB(req->szDeleteInfo);
	...
	else
	{
		...
		if (delpwcheck)
		{
			...
			if (0 != NTL_STRICMP(delpw[0].GetString(), md5pwd)) //check password
				resultcode = CHARACTER_DELETE_CHAR_FAIL_NOT_MATCH_CODE;
			else
			{
				smart_ptr<QueryResult> check = GetCharDB.Query("SELECT GuildID FROM characters WHERE CharID=%u AND AccountID=%u LIMIT 1", req->charId, GetPlayer()->GetAccountID());
				if (check)
				{
					Field* f = check->Fetch();
					if (f[0].GetUInt32() == 0)
	...
	else
	{
		...
		if (check)
		{
			...
			if (f[0].GetBool())
			{
				...
				smart_ptr<QueryResult> namecheck = GetCharDB.Query("SELECT CharID FROM characters WHERE CharName='%s' LIMIT 1", charname.c_str());
				if (!namecheck)
				{
