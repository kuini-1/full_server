		...
		for( int i = 0; i < MAX_SYSTEM_EFFECT_CODE; i++ )
		{
			...
			if( 0 == strcmp( buffer, pBuffer ) )
			{
				if( INVALID_TBLIDX != m_aEffectCodeMatchTable[ i ] )
				{
					...
					FormatStringToWCHAR(L"[File] : %s\n[Error] : Duplicated system effect found!(System Effect Name = %s)", wszFormatBuf, sizeof(wszFormatBuf) / sizeof(WCHAR));
					CTable::CallErrorCallbackFunction(wszFormatBuf, m_wszXmlFileName, pTbldat->wszName);
