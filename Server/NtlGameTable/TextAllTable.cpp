...
bool CTextAllTable::LoadFromBinary(CNtlSerializer& serializer, bool bReload, bool bUpdate)
{
	...
	while (0 < serializer.GetDataSize())
	{
		...
		{
			//- yoshiki : To log system!
			printf("if (serializer.GetDataSize() < sizeof(nTableType) + sizeof(nPayloadLength)) \n");
			Destroy();
			return false;
		}
		...
		{
			//- yoshiki : To log system!
			printf("if (TABLE_COUNT <= nTableType) nTableType %u\n", nTableType);
			Destroy();
			return false;
		}
		...
		else
		{
			...
			{
				//- yoshiki : To log system!
				printf("if (FALSE == pTable->Create(m_dwCodePage, TypeToField(nTableType), nTableType)) \n");
				delete pTable;
