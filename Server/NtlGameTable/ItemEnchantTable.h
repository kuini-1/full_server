...
public:
	...
	bool		ProcessRandomOption(sITEM_TBLDAT* itemtbl, BYTE byItemRank, bool bFirst, bool bLast, CNtlBitFlagManager* flagmgr, float* pfRemainingWorth, sITEM_RANDOM_OPTION& randomOption, TBLIDX kit, CHARACTERID player, BYTE isGM);
	bool		GenRandomOption(sITEM_TBLDAT* itemtbl, BYTE byItemRank, bool bFirst, bool bLast, CNtlBitFlagManager* flagmgr, float* pfRemainingWorth, sITEM_RANDOM_OPTION& randomOption, TBLIDX kit, CHARACTERID player, BYTE isGM); // by kuini testing options
	bool		UpgradeRandomOption(sITEM_TBLDAT* itemtbl, BYTE byItemRank, CNtlBitFlagManager* flagmgr, float* pfRemainingWorth, sITEM_RANDOM_OPTION& randomOption, TBLIDX kit); // by kuini testing options
...
private:
	...
	//kuini test dogiball increased rate
	struct sDOGIBALL_RATE
	{
		TBLIDX		item;
		float		rate;

		sDOGIBALL_RATE(TBLIDX party, float diffi)
			: item(party), rate(diffi)
		{
		}
	...
	std::map<CHARACTERID, sDOGIBALL_RATE*> m_map_DogiballRate;

	typedef std::multimap<CHARACTERID, sDOGIBALL_RATE*> DOGIBALLRATEGROUP;
	typedef DOGIBALLRATEGROUP::iterator DOGIBALLRATEGROUP_IT;
	typedef DOGIBALLRATEGROUP::value_type DOGIBALLRATEGROUP_VAL;

	DOGIBALLRATEGROUP							m_dogiballRateGroup;
