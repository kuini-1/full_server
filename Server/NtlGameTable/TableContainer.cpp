...
#include "MobServerTable.h"
#include "SpecialCharacterTable.h"
...
void CTableContainer::Init()
{
	...
	m_pContentsOnOffTable = NULL;
	m_pNpcServerTable = NULL;
	m_pMobServerTable = NULL;
	m_pSpecialCharacterTable = NULL;
...
bool CTableContainer::Create(CNtlBitFlagManager& rTableFlag, WCHAR* pwszPath, CTableFileNameList* pFileNameList, CTable::eLOADING_METHOD eLoadingMethod, DWORD dwCodePage, ICallBack* pCall)
{
	...
	if (false != rTableFlag.IsSet(TABLE_DROP_NORBAG))
	{
		DboCreateTable<CTableTmp<CDropTableEx::sTBLDAT_DropItem>>("CDropBagTable", serializer, pFileNameList->GetFileNameW(TABLE_DROP_NORBAG), &m_pDropBag, pCall);
	}
	if (false != rTableFlag.IsSet(TABLE_DROP_GRPBAG))
	{
		DboCreateTable<CTableTmp<CDropTableEx::sTBLDAT_DropBag>>("CDropBigBagTable", serializer, pFileNameList->GetFileNameW(TABLE_DROP_GRPBAG), &m_pDropBigBag, pCall);
	}
	...
	if (false != rTableFlag.IsSet(TABLE_CONTENTSONOFF))
	{
		DBO_CREATE_TABLE(CContentsOnOffTable, serializer, pFileNameList->GetFileNameW(TABLE_CONTENTSONOFF), m_pContentsOnOffTable, pCall);
	}
	...
	if (false != rTableFlag.IsSet(TABLE_SPECIAL_CHARACTER))
	{
		DBO_CREATE_TABLE(CSpecialCharacterTable, serializer, pFileNameList->GetFileNameW(TABLE_SPECIAL_CHARACTER), m_pSpecialCharacterTable, pCall);
	}
...
{
	printf("CTableContainer::Destroy() \n");
	m_wstrPath.clear();
	...
	if (NULL != m_pDropBigBag)
	{
		delete m_pDropBigBag;
		m_pDropBigBag = NULL;
	}
	if (NULL != m_pDwcTable)
	{
	...
	if (NULL != m_pContentsOnOffTable)
	{
		delete m_pContentsOnOffTable;
		m_pContentsOnOffTable = NULL;
	}
	...
	if (NULL != m_pSpecialCharacterTable)
	{
		delete m_pSpecialCharacterTable;
		m_pSpecialCharacterTable = NULL;
	}
...
bool CTableContainer::Reload(CNtlBitFlagManager& rTableFlag, CTableFileNameList* pFileNameList)
{
	...
	m_DropTableEx.Init(m_pDropBag, m_pDropBigBag);
...
//-----------------------------------------------------------------------------------
//	Purpose	: Update current loaded tables with the one from localize folder
//	Return	:
//-----------------------------------------------------------------------------------
...
bool CTableContainer::SaveToFile(CNtlBitFlagManager& rTableFlag, CTableFileNameList* pFileNameList, bool bNeedToEncrypt)
{
	...
	if (false != rTableFlag.IsSet(TABLE_CONTENTSONOFF))
	{
		DBO_EXPORT_TABLE(m_pContentsOnOffTable, serializer, pFileNameList->GetFileNameW(TABLE_CONTENTSONOFF), bNeedToEncrypt);
	}
	if (false != rTableFlag.IsSet(TABLE_NPC_SERVER))
	{
	...
	if (false != rTableFlag.IsSet(TABLE_SPECIAL_CHARACTER))
	{
		DBO_EXPORT_TABLE(m_pSpecialCharacterTable, serializer, pFileNameList->GetFileNameW(TABLE_SPECIAL_CHARACTER), bNeedToEncrypt);
	}
	if (false != rTableFlag.IsSet(TABLE_CONTENTS_CONDITION))
	{
