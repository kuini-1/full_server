...
#define AUTHKEY_KEY "s1f5dfg6sfgsd" //max 16
...
bool CNeighborServerInfoManager::RefreshServerFarmInfo(sDBO_GAME_SERVER_FARM_INFO *pServerFarmInfoRef)
{
	...
	//CNtlStringW msg;
	//msg.Format(name);
	printf("TEST 1: %s 2: \"%s\" 3: %s 4: \"%s\" \n", charname.c_str(), charname.c_str(), charname.c_str(), charname.c_str());
	//printf("TEST 1: %s 2: \"%ls\" \n", charname, charname);
	return true;
}
...
std::list<sDBO_SERVER_INFO* >* __unaligned CNeighborServerInfoManager::GetServerInfoList(SERVERFARMID serverFarmId, SERVERCHANNELID byServerChannelIndex, BYTE byServerType)
{
	switch (byServerType)
	{
	case NTL_SERVER_TYPE_MASTER:
...
sDBO_SERVER_INFO* CNeighborServerInfoManager::GetServerInfoHelper(std::list<sDBO_SERVER_INFO* >* __unaligned pServerInfoList, char* pszServerIP)
{
