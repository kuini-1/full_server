...
{
	std::unordered_map<ACCOUNTID, CClientSession*>::iterator it = m_map_Players.find(AccID);
...
bool CAuthServer::IsAccountTempBlocked(std::string & strUsername)
{
...
int main(int argc, _TCHAR* argv[])
{
	...
	if (!GetLogDB.Initialize(app.GetDatabaseHost(), 3306, app.GetDatabaseUser(),
		app.GetDatabasePassword(), app.GetDatabaseLog(), 5))
	{
