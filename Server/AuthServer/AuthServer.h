...
public:
	...
	virtual int		OnConfiguration(const char * lpszConfigFile)
	{
		...
		if (!file.Read("DATABASE_ACCOUNT", "Log", m_config.DatabaseLog))
		{
			return NTL_ERR_DBC_CONNECTION_CONNECT_FAIL;
		}
...
public:
	...
	bool			IsAccountTempBlocked(std::string& strUsername);
	void			RegisterAccountTempBann(std::string& strUsername);
