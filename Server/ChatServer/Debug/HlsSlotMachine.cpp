...
void CHlsSlotMachine::Init()
{
	...
	for (CTable::TABLEIT it = pItemTable->Begin(); it != pItemTable->End(); it++)
	{
		...
		sHLS_ITEM_TBLDAT* pHlsItem = (sHLS_ITEM_TBLDAT*)g_pTableContainer->GetHLSItemTable()->FindData(pHlsSlotItem->tblidx);
		if(pHlsItem)
		{
			...
			for (CTable::TABLEIT it = pSlotTable->Begin(); it != pSlotTable->End(); it++)
			{
				...
				{
					if (pHlsMachineTbldat->aItemTblidx[i] == pHlsSlotItem->tblidx)
					{
