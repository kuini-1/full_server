...
private:
	...
	WCHAR						m_wLastMsg[NTL_MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE + 1];
	int							m_nMsgSpamCount;
...
public:
	...
	void			LoadWaguPointDBProc(QueryResultVector& results);
	...
	void									SetCanLoadWaguPoint(bool bFlag) { m_bCanLoadWaguPoint = bFlag; }
	bool									GetCanLoadWaguPoint() { return m_bCanLoadWaguPoint; }
	...
	inline void								SetWaguWaguPoints(DWORD cash) { m_dwWaguWaguPoints = cash; if (GetWaguWaguPoints() > NTL_MAX_WAGU_WAGU_SHOPPOINTS)
																SetWaguWaguPoints(NTL_MAX_WAGU_WAGU_SHOPPOINTS); }
...
private:
	...
	WCHAR									m_awchGuildName[NTL_MAX_SIZE_GUILD_NAME + 1];
	GUILDID									m_GuildID;
	CHARACTERID								m_GuildInvitedByID;

public:

	WCHAR*									GetGuildName()		{ return m_awchGuildName; }
