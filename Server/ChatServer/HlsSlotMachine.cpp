	...
	for (CTable::TABLEIT it = pItemTable->Begin(); it != pItemTable->End(); it++)
	{
		...
		if (pHlsItem)
		{
			...
			for (CTable::TABLEIT it = pSlotTable->Begin(); it != pSlotTable->End(); it++)
			{
				...
				{
					if (pHlsMachineTbldat->aItemTblidx[i] == pHlsSlotItem->cashItemTblidx && pHlsMachineTbldat->tblidx == pHlsSlotItem->slotMachineTblidx)
					{
						pSlot->wCountLeft = pHlsMachineTbldat->wQuantity[i];//1;
						pSlot->byRank = i + 1;
						pSlot->fPercent = (float)pHlsSlotItem->byPercent;// 0.0f;
			...
			if (isWinnerItem == false)
			{
				pSlot->wCountLeft = INVALID_WORD;
				pSlot->byRank = 0;
				pSlot->fPercent = (float)pHlsSlotItem->byPercent;
				pSlot->pHlsItem = pHlsItem;
				pSlot->pSlotItem = pHlsSlotItem;

				m_slotMachineGroup.insert(SLOTMACHINEGROUP_VAL(0, pSlot));
			}
...
void CHlsSlotMachine::GetSlotItems(TBLIDX slotIdx, std::vector<sHLS_SLOT_ITEM*>* pVec)
{
	...
	std::multimap<TBLIDX, sHLS_SLOT_ITEM*>::iterator itLow2 = m_slotMachineGroup.lower_bound(0);
	std::multimap<TBLIDX, sHLS_SLOT_ITEM*>::iterator itUp2 = m_slotMachineGroup.upper_bound(0);
	...
	while (itLow != itUp)
	{
		...
		{
			if (Dbo_CheckProbabilityF((float)pTbldat->fPercent + float(pSlot->wMaxCapsule / pSlot->wCurrentCapsule)))
	...
	while (itLow2 != itUp2)
	{
		sHLS_SLOT_ITEM* pTbldat = itLow2->second;
