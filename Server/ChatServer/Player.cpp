...
void CPlayer::LoadFromDB()
{
	...
	if (m_bCanLoadWaguPoint)
	{
		m_bCanLoadWaguPoint = false;

		SQLCallbackBase* pCallBack = new SQLClassCallbackP0<CPlayer>(this, &CPlayer::LoadWaguPointDBProc);
		AsyncQuery* q = new AsyncQuery(pCallBack);
		q->AddQuery("SELECT WaguPoint FROM characters WHERE CharID=%u", GetCharID()); 
		GetCharDB.QueueAsyncQuery(q);
	}
}
...
void CPlayer::LoadWaguPointDBProc(QueryResultVector& results)
{
	if (g_pPlayerManager->FindPlayerWithCharID(GetCharID()) == NULL)
	{
		ERR_LOG(LOG_USER, "Player %u not found in player map", GetCharID());
	...
	if (results[0].result)
	{
		...
		m_dwWaguWaguPoints = f[0].GetDWORD();
	}
}
...
void CPlayer::SetGuild(CGuild* _guild)
{
	...
	else
	{
		SetGuildID(0);
		m_pGuild = NULL;
		::ZeroMemory(m_awchGuildName, NTL_MAX_SIZE_GUILD_NAME + 1);
	}
}
