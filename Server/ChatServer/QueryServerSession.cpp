...
void CQueryServerSession::RecvGuildFunctionAddRes(CNtlPacket * pPacket, CChatServer * app)
{
	...
	if (req->wResultCode == GAME_SUCCESS)
	{
		...
		if (pGuild)
		{
			...
			::ZeroMemory(res->wszGuildName, NTL_MAX_SIZE_GUILD_NAME_IN_UNICODE + 1);
...
void CQueryServerSession::RecvAuctionHouseSellRes(CNtlPacket * pPacket, CChatServer * app)
{
	...
	if (player)
	{
		...
		res->dwFee = req->dwFee;
		res->dwPrice = req->sData.dwPrice;
		res->itemId = req->sData.itemId;
		res->nEndSellTime = req->sData.nEndSellTime;
		res->nItem = req->sData.nItem;
		res->nStartSellTime = req->sData.nStartSellTime;
		res->wResultCode = req->wResultCode;
...
void CQueryServerSession::RecvAuctionHouseSellCancelRes(CNtlPacket * pPacket, CChatServer * app)
{
	...
	if (req->nItem == INVALID_ITEMID)
	{
		g_pAH->DeleteAllItems(req->charId);
...
void CQueryServerSession::RecvAuctionHouseBuyRes(CNtlPacket * pPacket, CChatServer * app)
{
	...
	if (player)
	{
		...
		res->itemID = req->itemID;
		res->nItem = req->nItem;
		res->bSellerServer = false;
		res->dwMoney = req->dwMoney;
		res->mailID = req->mailId;
		res->sellcharId = req->sellcharId;
		res->sellmailID = req->sellmailId;
		res->wResultCode = req->wResultCode;
...
void CQueryServerSession::RecvHlsSlotMachineExtractRes(CNtlPacket * pPacket, CChatServer * app)
{
	...
	if (player)
	{
		...
		if (req->wResultCode == CHAT_SUCCESS)
		{
			...
			{
				player->SetCanLoadWaguPoint(true);
				player->LoadFromDB();

				//player->SetWaguWaguPoints(player->GetWaguWaguPoints() + req->wCoin);
				res->wNewWaguWaguPoints = player->GetWaguWaguPoints() + req->wCoin; //TODO
			}
		}
		...
		if (req->wResultCode == CHAT_SUCCESS)
		{
			...
			for (BYTE i = 0; i < req->byExtractCount; i++)
			{
				...
				res2->sInfo.giftCharId = INVALID_CHARACTERID;
				res2->sInfo.HLSitemTblidx = req->ItemTblidx[i];
				res2->sInfo.nIsRead = FALSE;
			...
			resEnd->dwGiftPoint = player->GetWaguWaguPoints();
