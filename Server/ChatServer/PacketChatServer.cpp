...
void CClientSession::RecvChatFindParty(CNtlPacket * pPacket)
{
	...
	//if (cPlayer->GetLevel() < 5) // disabled by kuini lfp limit level
	//	return; 
...
void CClientSession::RecvHlsSlotMachineExtractReq(CNtlPacket * pPacket)
{
	...
	if (pSlotMachine)
	{
		if (pSlotMachine->pTbldat->byType == HLS_MACHINE_TYPE_WAGUWAGU ||
			pSlotMachine->pTbldat->byType == HLS_MACHINE_TYPE_EVENT)
		{
			if (req->byExtractCount <= DBO_MAX_HLS_SLOT_MACHINES_MAX_ITEMS)
			{
				if (pSlotMachine->wCurrentCapsule >= req->byExtractCount)
				{
					DWORD dwHaveCoin = (pSlotMachine->pTbldat->byType == HLS_MACHINE_TYPE_EVENT)
						? cPlayer->GetEventMachineCoin()
						: cPlayer->GetWaguMachineCoin();

					if (dwHaveCoin >= DWORD(pSlotMachine->pTbldat->byCoin * req->byExtractCount)) //check if enough coins
					{
						...
						if (vecSlotItems.size() > 0)
						{
							...
							res2->byHlsMachineType = pSlotMachine->pTbldat->byType; // 0 = slot, 1 = event
							...
							for (BYTE i = 0; i < req->byExtractCount; i++)
							{
								if (vecSlotItems.size() > 0)
								{
									...
									{
										for (BYTE count = 0; count < DBO_MAX_HLS_SLOT_MACHINES_MAX_ITEMS; count++)
										{
											if (pSlotItem->pHlsItem->tblidx == pSlotMachine->pTbldat->aItemTblidx[count])
												res2->abyStackCount[i] = pSlotMachine->pTbldat->byStack[count];
										}

										if (pSlotItem->byRank == 1)
										{
	...
	cPlayer->SetCanLoadWaguPoint(true);
	cPlayer->LoadFromDB();

	if (pSlotMachine->pTbldat->byType == HLS_MACHINE_TYPE_WAGUWAGU)
	{
		cPlayer->SetWaguWaguPoints(cPlayer->GetWaguWaguPoints() + req->byExtractCount);
		res->wNewWaguWaguPoints = cPlayer->GetWaguWaguPoints();
	}
	else
	{
		cPlayer->SetEventMachineCoin(cPlayer->GetEventMachineCoin() + req->byExtractCount);
		res->wNewWaguWaguPoints = cPlayer->GetEventMachineCoin();
	}
