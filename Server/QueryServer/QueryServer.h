...
public:
	...
	CNtlString		GetDatabaseLog() { return m_config.DatabaseLog; }
	...
	inline bool		ReserveName() { return m_config.bReserveName; }
	...
	int	OnConfiguration(const char * lpszConfigFile)
	{
		...
		if (!file.Read("DATABASE_CHARACTER", "Log", m_config.DatabaseLog))
		{
			return NTL_ERR_DBC_CONNECTION_CONNECT_FAIL;
		}
		//DATABASE_ACCOUNT
		...
		if (!file.Read("CONFIG", "EnableReserveName", m_config.bReserveName))
			return NTL_ERR_SYS_CONFIG_FILE_READ_FAIL;
