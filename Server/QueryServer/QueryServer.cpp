...
int CQueryServer::OnAppStart()
{
	...
	if (m_config.bReserveName)
	{
		NTL_PRINT(PRINT_APP, "Create Reserve Name Manager");

		CReserveName* reserveName = new CReserveName;
		UNREFERENCED_PARAMETER(reserveName);
	}
...
BOOL CQueryServer::OnCommandInput(std::string& sCmd)
{
	...
	else if (sCmd == "reloadreservename")
	{
		g_pReserveName->ReloadFile();
	}
...
int main(int argc, _TCHAR* argv[])
{
	...
	int rc = app.Create(argc, argv, (argc > 1) ? argv[1] : "./config/QueryServer.ini");
	if (NTL_SUCCESS != rc)
		return rc;
...
#else
	if (ti_ptr)
		snprintf(m_LogFile, sizeof(m_LogFile), "./logs/queryserver/log_%02d-%02d-%02d.txt", ti_ptr->tm_year + 1900, ti_ptr->tm_mon + 1, ti_ptr->tm_mday);
	else
		snprintf(m_LogFile, sizeof(m_LogFile), "./logs/queryserver/log_00-00-00.txt");
#endif
	...
	if (!GetLogDB.Initialize(app.GetAccDbHost(), 3306, app.GetAccDbUser(),
		app.GetAccDbPass(), app.GetDatabaseLog(), 2))
	{
