...
void CAccountCache::OnLoadAccountInfo(QueryResultVector & results)
{
	...
	if (qrAccountInfo)
	{
		...
		m_sAccNewData.bVipLevel = f[5].GetBYTE(); //by kuini
	}
...
void CAccountCache::OnCashShopStorage(QueryResultVector& results)
{
	QueryResult* qrCashItemInfo = results[0].result;
	if (qrCashItemInfo)
	{
		m_mapCashItems.clear();
		do
		{
			Field* f = qrCashItemInfo->Fetch();
...
void CPlayerCache::Init()
{
	...
	m_CanLoadCache = true; // by kuini can load cache
...
void CPlayerCache::OnLoadPcData(QueryResultVector & results)
{
	...
	if (qrCharacterInfo)
	{
		...
		m_sPcNewData.dwSpPointBought = f[59].GetDWORD(); // by kuini for SP purchase

		m_sPcData.charTitle = f[46].GetUInt32();

		m_sPcData.sMixData.bNormalStart = f[41].GetBool();
		m_sPcData.sMixData.bSpecialStart = f[42].GetBool();
		m_sPcData.sMixData.bySpecialType = f[43].GetBYTE();
		m_sPcData.sMixData.byMixLevel = f[44].GetBYTE();
		m_sPcData.sMixData.dwMixExp = f[45].GetDWORD();

		m_sPcData.sLocalize.pcProfileSDCH.nRecvGiftCount = 0;
		m_sPcData.sLocalize.pcProfileSDCH.timeP = f[49].GetDWORD();
		m_sPcData.sLocalize.pcProfileSDCH.byPlayRestrict = 0;

		m_sPcData.dwWaguWaguPoints = f[50].GetDWORD();

		m_sPcData.bySuperiorEffect = 0;
		m_sPcData.bySuperiorType = 0;
		m_sPcData.bySuperiorValue = 0;
...
void CPlayerCache::SetNetpy(DWORD dwPoints)
{
	m_sPcData.sLocalize.pcProfileSDCH.timeP = dwPoints;
...
void CPlayerCache::SetWaguPoints(DWORD dwPoints)
{
	...
	if (m_sPcData.dwWaguWaguPoints > NTL_MAX_WAGU_WAGU_SHOPPOINTS)
		m_sPcData.dwWaguWaguPoints = NTL_MAX_WAGU_WAGU_SHOPPOINTS;
}
...
{
	m_sPcData.sMixData.byMixLevel = byLevel;
	m_sPcData.sMixData.dwMixExp = dwExp;
}
...
void CPlayerCache::InitHtbSkills()
{
	std::map<TBLIDX, sHTB_SKILL_DATA*>::iterator it = m_mapHtbSkills.begin();
	while (it != m_mapHtbSkills.end())
	{
		sHTB_SKILL_DATA* pSkill = it->second;
...
TBLIDX CPlayerCache::SwitchItem(ITEMID itemId, CPlayerCache * pNewOwner, BYTE byNewPlace, BYTE byNewPos)
{
	std::map<ITEMID, sITEM_DATA*>::iterator it = m_mapItems.find(itemId);
	if (it != m_mapItems.end())
	{
...
void CPlayerCache::StartMailResult(QueryResultVector & results, HOBJECT hHandle, HOBJECT hObject, eSCOUTER_PARTS_TYPE ePartType)
{
	if (results[0].result)
	{
...
bool CPlayerCache::CanLoadCache()  // by kuini can load cache
{
	return m_CanLoadCache;
}

void CPlayerCache::SetCanLoadCache(bool canLoad)  // by kuini can load cache
{
	m_CanLoadCache = canLoad;
}

bool CPlayerCache::ClassChange()  // by kuini TESTING
{
	return m_ClassChange;
}

void CPlayerCache::SetClassChange(bool set)  // by kuini TESTING
{
	m_ClassChange = set;
}
...
{
	for (std::unordered_map<ACCOUNTID, CAccountCache*>::iterator it = m_mapAccounts.begin(); it != m_mapAccounts.end(); )
	{
		delete it->second;
	...
	for (std::unordered_map<CHARACTERID, CPlayerCache*>::iterator it = m_mapCharacters.begin(); it != m_mapCharacters.end(); )
	{
		delete it->second;
...
{
	std::unordered_map<ACCOUNTID, CAccountCache*>::iterator it = m_mapAccounts.find(accountID);
...
{
	std::unordered_map<CHARACTERID, CPlayerCache*>::iterator it = m_mapCharacters.find(charID);
