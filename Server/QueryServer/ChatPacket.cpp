	...
	if (pCache)
	{
		...
		if (pCharCache)
		{
			if (pCache->GetWaguCoin() >= (DWORD)req->wCoin)
			{
				SYSTEMTIME ti;
				GetLocalTime(&ti);
				int productId = g_pCashshopManager->AcquireProductId();
				...
				pCharCache->SetWaguPoints(pCharCache->GetWaguPoints() + res->byExtractCount); 
				GetCharDB.WaitExecute("UPDATE characters SET WaguPoint=WaguPoint+%u WHERE CharID=%u", res->byExtractCount, req->charId);
			}
			else res->wResultCode = QUERY_FAIL;
		}
		else res->wResultCode = QUERY_FAIL;
