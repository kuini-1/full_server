...
public:
	...
	void					OnCashShopStorage(QueryResultVector& results);
...
public:
	...
	sACC_NEW_DATA			GetAccNewData() { return m_sAccNewData; }
...
private:
	...
	//by kuini for account db to game server
	sACC_NEW_DATA							m_sAccNewData; // by kuini for account db to game server
};
...
public:
	...
	void						SetNetpy(DWORD dwPoints);
	DWORD						GetNetpy() { return m_sPcData.sLocalize.pcProfileSDCH.timeP; }
	...
	TBLIDX						SwitchItem(ITEMID itemId, CPlayerCache* pNewOwner, BYTE byNewPlace, BYTE byNewPos);
	...
	void						StartMailResult(QueryResultVector & results, HOBJECT hHandle, HOBJECT hObject, eSCOUTER_PARTS_TYPE ePartType);
	...
	bool						CanLoadCache();  // by kuini can load cache
	void						SetCanLoadCache(bool canLoad);
	bool						ClassChange();  
	void						SetClassChange(bool set);
...
private:
	...
	bool									m_CanLoadCache; // by kuini can load cache
	bool									m_ClassChange;
