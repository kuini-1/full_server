...
void CCharServerSession::RecvCreateCharReq(CNtlPacket * pPacket, CQueryServer* app)
{
	...
	else
	{
		...
		{
			GetCharDB.Execute("INSERT INTO quickslot (CharID,Tblidx,Slot,Type,Item) VALUES (%u,%u,%u,%u,%u)", newCharId, req->asQuickSlotData[i].tbilidx, req->asQuickSlotData[i].byQuickSlot, req->asQuickSlotData[i].byType, 0);
		}
...
void CCharServerSession::RecvCharacterLoadReq(CNtlPacket* pPacket, CQueryServer* app)
{
	...
	if (pAccount)
	{
		...
		SQLCallbackBase* pCallBack = new SQLClassCallbackP0<CAccountCache>(pAccount, &CAccountCache::OnCashShopStorage);
	...
	else
	{
		...
		AsyncQuery* q = new AsyncQuery(pCallBack);
		q->AddQuery("SELECT mallpoints, isGm, PremiumSlots, EventCoins, WaguCoins, vip FROM accounts WHERE AccountID=%u", req->accountId);
		q->AddQuery("SELECT * FROM cashshop_storage WHERE AccountID=%u AND isMoved=0", req->accountId);
